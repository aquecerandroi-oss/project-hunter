...
7. **Brainstorm → plan → implement → review.** Classify every request as *spike* (answer, no permanent code), *bounded* (≤ 3 files inside an existing flow: two-sentence design in chat, approval, implement) or *architectural* (new subsystem or interface change: design doc in `docs/`, explicit approval, then a plan in `docs/plans/`). When in doubt, pick the heavier path. Never downgrade mid-task.
