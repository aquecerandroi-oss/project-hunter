...
export function ClosedTodaySection({ orgSlug, rows }: ClosedTodaySectionProps) {
  return (
      ...
      {rows.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {rows.map((row) => {
            ...
            if (!bet) return null;
            return (
              <li key={bet.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border p-3 text-xs">
...
export function RecentDecisionsSection({ orgSlug, rows, limit = 12 }: RecentDecisionsSectionProps) {
      ...
      {shown.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1">
          {shown.map((row) => {
            ...
            return (
              <li key={row.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border px-3 py-2 text-xs">
                <span className="min-w-0">
