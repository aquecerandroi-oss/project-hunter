...
const REAL_EXAMPLE = {
  ...
  fx_reason: null,
  series_30d: [{ day: "2026-08-08", unique_r: "0", pooled_r: "0", unique_usdt: "0" }],
};
...
describe("dailyGoalOutSchema: parses the frozen contract's real example verbatim", () => {
  ...
  it("round-trips series_30d as an array of Decimal-string points", () => {
    ...
    expect(parsed.series_30d[0]).toEqual({ day: "2026-08-08", unique_r: "0", pooled_r: "0", unique_usdt: "0" });
  });

  it("parses the fx block (T3.78b) verbatim -- rate, source, both timestamps", () => {
    const parsed = dailyGoalOutSchema.parse(REAL_EXAMPLE);
    expect(parsed.fx).toEqual({
      rate: "5",
      source: "binance.spot.ticker",
      observed_at: "2026-09-05T00:00:00Z",
      available_at: "2026-09-05T00:00:00Z",
    });
    expect(parsed.value_of_1r.real_usdt_p50).toBe("28");
    expect(parsed.progress.real_usdt).toBe("28");
  });
});

describe("dailyGoalOutSchema: null-with-reason branches (FX/sizing absent)", () => {
  it("accepts every real_brl_*/real_usdt_* null together with a reason, sample_size 0, fx null", () => {
    const noFx = {
      ...REAL_EXAMPLE,
      value_of_1r: {
        label_brl: "250.00",
    ...
    expect(parsed.value_of_1r.real_usdt_p50).toBeNull();
    expect(parsed.progress.real_brl).toBeNull();
    expect(parsed.progress.real_usdt).toBeNull();
    expect(parsed.fx).toBeNull();
    expect(parsed.fx_reason).toBe("no_fx_observation");
  });

  it("accepts real_usdt_* populated while real_brl_*/fx stay null (USDT never needs an FX rate)", () => {
    const usdtOnly = {
      ...REAL_EXAMPLE,
      value_of_1r: {
        label_brl: "250.00",
      ...
      progress: { ...REAL_EXAMPLE.progress, real_brl: null, real_usdt: "28" },
      fx: null,
      fx_reason: "no_fx_observation",
    };
    const parsed = dailyGoalOutSchema.parse(usdtOnly);
    expect(parsed.value_of_1r.real_usdt_p50).toBe("28");
    expect(parsed.value_of_1r.reason).toBeNull();
    expect(parsed.progress.real_usdt).toBe("28");
    expect(parsed.progress.real_brl).toBeNull();
    expect(parsed.fx).toBeNull();
  });
