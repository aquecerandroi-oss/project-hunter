...
describe("nav-registry", () => {
  ...
  it("marks dashboard, markets, lab, radar, opportunities, portfolio, system and settings available (M1: markets/lab go live; T2.7: radar/opportunities go live; T3.8b: portfolio goes live)", () => {
    const available = NAV_ITEMS.filter((item) => item.status === "available").map((item) => item.key);
    expect(available.sort()).toEqual(["dashboard", "lab", "markets", "opportunities", "portfolio", "radar", "settings", "system"]);
  });
  ...
  it("hides planned items in production", () => {
    ...
    expect(items.map((item) => item.key).sort()).toEqual(["dashboard", "lab", "markets", "opportunities", "portfolio", "radar", "settings", "system"]);
  });
