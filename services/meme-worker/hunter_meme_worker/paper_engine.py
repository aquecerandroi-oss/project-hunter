...
1. **A fill is priced against a snapshot observed strictly after the decision**
   ...
   fill at the price that motivated the decision.
...
curve's own events, the loss floor, the target, the trailing, the time stop);
the operator's ``sell_now`` sits above all of them, because an explicit human
order is not a rule to be outranked.
"""
...
def _refuse_fill(
    ...
    *,
    ...
    if wallet.open_positions >= spec.max_open_positions:
...
def evaluate_fill(
    ...
    *,
    ...
    reason = _refuse_fill(spec, params, wallet, snapshot, decided_at=decided_at, migrated=migrated)
    if reason is not None:
        return FillVerdict("refused", reason)
    try:
        entry = _build_entry(spec, params, snapshot, decided_at=decided_at, sol_usd=sol_usd)
    except ValueError:
