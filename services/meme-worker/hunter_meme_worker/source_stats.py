"""One upstream source's own shape: a sliding-window counter and the
request/response stats ``GET /meme/sources`` renders per source.

Split out of ``sources.py`` in T4.16b for the 350-line budget — that module
was already at the ceiling before the tracker's pinned set needed two more
heartbeat fields. Nothing here knows about the radar-wide gauges
(``SourcesState`` folds these together); a :class:`SourceStats` is one
source's own liveness, and a :class:`RollingCounter` is the sliding window
every per-minute/per-hour count in this worker is built from.
"""
...
__all__ = ["RollingCounter", "SourceStats"]
...
class SourceStats:
    ...
    def as_fields(self, now: datetime) -> dict[str, object]:
        ...
        return {
            ...
            "last_received_at": iso_or_none(self.last_received_at),
