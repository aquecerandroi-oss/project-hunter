"""The per-minute cadence around :func:`run_dispersion_once`, and the hand crank.

T3.90. Two entry points, one body — the shape ``breadth.py``, ``beta.py`` and
``regime_hourly.py`` already use, for the same reasons:

- :func:`dispersion_loop` runs inside the scanner's TaskGroup and produces the
  minute that just closed (plus any of the last three hours that have no row);
- ``python -m hunter_scanner_worker.dispersion --once`` runs exactly one pass and
  exits — for a first fill, for an operator repairing a hole, and for the proof a
  task owes. Ninety days is not this crank's job: that is
  ``infra/scripts/backfill_dispersion.py``, which is audited and dry-run by
  default.

**One producer per exchange per minute, and the lock names the minute.** The key is
``dispersion:producer:{exchange}:{cut}`` (``SET NX``, TTL 10 min, no release).
Correctness does not depend on it — ``uq_market_dispersion_reading`` makes a
duplicate pass a no-op at the database — so this is economy, not integrity, and a
leader that dies mid-pass simply leaves the minute to the next pass, which sees it
as missing. That is the whole recovery story, and it is the reason the pass has no
repair rule to get wrong.
...
LOCK_TTL_S = 600
"""Ten minutes: far longer than a steady-state pass (two candles per market) and
far shorter than the interval at which stale keys would accumulate."""

__all__ = ["CHECK_S", "DispersionHealth", "claim_minute", "dispersion_loop", "main"]
"""``DispersionHealth`` is re-exported so the scanner's ``main`` imports the loop
and the health object it feeds from one place; the module the dataclass lives in is
not ``main``'s business."""


def _lock_key(exchange: str, cut: datetime) -> str:
    return f"dispersion:producer:{exchange}:{cut.isoformat()}"
...
async def claim_minute(redis: redis_asyncio.Redis, exchange: str, cut: datetime) -> bool:
    ...
    A Redis failure answers **no**: the minute is not lost (the next pass sees it as
    missing and produces it), and not folding twice is the cheaper mistake.
    """
    try:
        return bool(await redis.set(_lock_key(exchange, cut), "1", nx=True, ex=LOCK_TTL_S))
    except Exception:
        logger.warning("scanner_dispersion_lock_unavailable", cut=cut.isoformat())
        return False


async def dispersion_loop(
    factory: async_sessionmaker[AsyncSession],
    redis: redis_asyncio.Redis,
    runtime: WorkerRuntime,
    health: DispersionHealth,
    *,
    exchange: str,
    ...
    while True:
        ...
        if (computed is None or cut > computed) and await claim_minute(redis, exchange, cut):
            computed = cut
            try:
                run = await run_dispersion_once(factory, exchange=exchange, cut=cut, back=back)
                ...
                runtime.mark_error()
                logger.exception("scanner_dispersion_pass_failed", cut=cut.isoformat())
        await asyncio.sleep(CHECK_S)


async def _run_once(exchange: str, *, back: int) -> DispersionRun:
...
def main() -> int:
    ...
    (``scanner_dispersion_pass``) plus ``scanner_dispersion_once``: a worker package
    may not ``print`` (ruff ``T201``), and duplicating the numbers on stdout would
    give an operator two places to read one fact.
    """
    parser = argparse.ArgumentParser(description="Produce the per-minute dispersion_24h rows.")
    parser.add_argument(
