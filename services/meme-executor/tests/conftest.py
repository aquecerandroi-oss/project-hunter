"""Testcontainers fixtures for the meme-executor — mirrors ``services/meme-worker/tests/conftest.py``.

Skips cleanly (reason logged) when Docker is unreachable, the rule the whole
repo's integration suite follows.
"""
...
def migrated_db_url(postgres_container: PostgresContainer) -> Iterator[str]:
    """A database in the shared container at ``head`` — with ``0028`` applied."""
    url = asyncio.run(
        _create_database(postgres_container.get_connection_url(), "hunter_meme_executor")
    )
