"""Testcontainers fixtures for the execution-worker — mirrors
``services/scanner-worker/tests/conftest.py``.

Skips cleanly (reason logged) when Docker is unreachable. One database of its
own (``hunter_execution_worker``): these tests open a paper wallet, and a wallet
is permanent per organization for ever, so sharing a database with another
suite would let one module's opening decide another module's ``WalletAlreadyOpen``.
"""
...
def migrated_db_url(postgres_container: PostgresContainer) -> Iterator[str]:
    """A database in the shared container with every migration applied (0007)."""
    url = asyncio.run(
        _create_database(postgres_container.get_connection_url(), "hunter_execution_worker")
    )
