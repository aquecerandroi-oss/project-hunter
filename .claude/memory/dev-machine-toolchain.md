...
Windows 11 dev machine, toolchain installed on 2026-09-04. Shells opened before an install do not see new PATH entries: in the Bash tool prepend `/c/Program Files/nodejs:/c/Users/evert/AppData/Roaming/npm:/c/Users/evert/AppData/Local/Microsoft/WinGet/Packages/astral-sh.uv_Microsoft.Winget.Source_8wekyb3d8bbwe:/c/Users/evert/.local/bin` to PATH, or ask the user to restart the app. Locations: Node 24 `C:\Program Files\nodejs`; pnpm 11 `%APPDATA%\npm` (installed with `npm -g`, not corepack); uv 0.12 in the WinGet Packages dir above; Python 3.12.14 managed by uv (`uv run` always works; bare `python` may still hit the Microsoft Store alias). Docker Desktop 4.89 is installed per-user (`%LOCALAPPDATA%\Programs\DockerDesktop\resources\bin` — add it to PATH in stale shells too, otherwise `docker pull` fails with "docker-credential-desktop not found"). It runs on WSL 2.7 (`docker-desktop` distro); verified with `hello-world` on 2026-09-04. The engine only runs while the Docker Desktop app is open: if `docker info` says "unable to start" or "backend is not running", start (or kill and restart) `Docker Desktop.exe` and wait ~10 s. `gh` is still absent. Hooks and `verify.mjs` were executed successfully with Node.
