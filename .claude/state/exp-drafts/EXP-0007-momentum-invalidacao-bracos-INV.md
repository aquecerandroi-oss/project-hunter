...
> Rascunho da T3.32 para arquivamento pela Sexta-feira. Nome de arquivo em ASCII de propósito
> (`bracos`, sem cedilha) para não depender de normalização de acento no vault.
...
braços da [[KB-0006]] muda o sinal da expectancy. Contra-hipótese pré-registrada da [[KB-0010]]: a
diferença é indistinguível de zero e/ou troca de sinal entre populações.
...
- **Método:** replay de **política de saída** sobre entradas **congeladas**, não estratégia nova.
  ...
  [[KB-0006]] pede (`Δ = média_i(R_i^alt − R_i^atual)`) e que uma `momentum_v2` prospectiva **não**
  produziria (ela re-arma sozinha e misturaria seleção de entrada com saída — T3.26/T3.27).
...
- **Conclusion:**
  1. **A invalidação não é a causa da perda.** Os três braços INV ficam entre −0,070 R e +0,032 R
     ...
     populações. A [[KB-0006]] está certa ao descrever (a invalidação encerra 26–43 % das operações
     com ~0 ganhadoras) e **errada ao concluir que soltá-las salvaria dinheiro**: soltas, elas
     perdem igual. Isto **encerra o item 0 do brief T3.27** — a versão de código `momentum_v2` com
     `invalidation_mode` **não se justifica** por este resultado.
  2. **O maior contraste é o alvo, e ele não é confiável.** `EXIT-NOTGT − base` é +0,163 R em P1 e
     ...
     armadilha da [[KB-0010]]. Não há decisão a tomar aqui sem uma janela reservada.
  3. **Nenhum braço salva a `volume_anomaly`.** O melhor (`EXIT-CHAN`) ainda entrega −0,419 R, porque
     o custo assumido dela vale 0,615 R por operação ([[KB-0076]]). Política de saída não conserta
     um custo de 0,6 R.
...
|---|---|---|---|
| `momentum v4` (`atr_pct_min 0,003 → 0,0089`) | 2026-09-08 13:05Z | piso de custo ([[KB-0008]]) | [[EXP-0006]] |
...
[[EXP-0004]] · [[EXP-0006]] · [[KB-0006]] · [[KB-0008]] · [[KB-0010]] · [[KB-0076]] ·
