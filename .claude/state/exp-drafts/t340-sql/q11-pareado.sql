-- T3.40 — os quatro grupos pareados por (mercado, barra), no metodo da T3.26.
-- Somente leitura; um snapshot para as quatro consultas.
\set pop 'select s.key||\' \'||sv.version as versao, left(split_part(o.meta->>\'cohort\',\':\',2),8) as coorte, a.market_id, (o.meta->\'entry_plan\'->>\'source_bar_close\')::timestamptz as bar, o.result::text as motivo, o.tracking_state::text as estado, o.r_multiple as r_net, (o.meta->>\'r_ex_funding\')::numeric as r_exf, (o.meta->\'progress\'->>\'entry\')::numeric as p_entry, (o.meta->\'progress\'->>\'exit_base\')::numeric as exit_base, (o.meta->\'excursions\'->>\'initial_risk\')::numeric as risk, (a.supporting_features->\'atr\'->>\'percent\')::numeric as atr_pct from signal_outcomes o join agent_signals a on a.id=o.signal_id join strategy_versions sv on sv.id=a.strategy_version_id join strategies s on s.id=sv.strategy_id where o.meta->>\'cohort\' in (\'replay:f8d8279c-1fba-42ae-95ef-202042f96c60\',\'replay:72cf5671-ec16-4d62-afd6-ace3f7bbf4e1\',\'replay:9a08835a-ae13-4c23-b521-734b2f60a3a2\')'
begin transaction isolation level repeatable read read only;
\pset border 2
\pset numericlocale off
select now() as read_at;

-- V1: a populacao do pai partida pelo piso 0,020 + a contagem da variante
with pop as (:pop)
select 'V1' as variante,
       case when atr_pct >= 0.020 then 'pai ACIMA do piso 0,020'
            else 'pai ABAIXO do piso (eliminadas)' end as grupo,
       count(*) as n, count(*) filter (where r_net is not null) as avaliaveis,
       round(avg((exit_base - p_entry/1.0006)/risk), 4) as exp_bruta_r,
       round(avg(r_net), 4) as exp_liquida_r, round(sum(r_net), 2) as soma_r
  from pop where coorte='f8d8279c' group by 1,2 order by 2 desc;

with pop as (:pop)
select 'variante V1 (momentum v5)' as lado, count(*) as decisoes from pop where coorte='72cf5671';

-- V2: os quatro grupos pareados
with pop as (:pop),
     pai as (select * from pop where coorte='f8d8279c'),
     var as (select * from pop where coorte='9a08835a'),
     g as (
...
select grupo, count(*) as n, count(*) filter (where r_net is not null) as avaliaveis,
       round(avg((exit_base - p_entry/1.0006)/risk), 4) as exp_bruta_r,
...
-- V2: o delta pareado, sinal a sinal
with pop as (:pop)
select count(*) as pares,
 ...
 where p.coorte='f8d8279c' and v.coorte='9a08835a' and p.r_net is not null and v.r_net is not null;
commit;
