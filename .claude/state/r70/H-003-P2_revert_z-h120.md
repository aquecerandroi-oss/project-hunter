# H-003 — P2_revert_z em h = 120 min (lado à vista, universo U2) — NÃO CONFIRMA

> Origem: R68 parte B — h=240 ficou inconclusivo em 5 células (partb_U2.txt 28–32: IC atravessando zero em todas) · KB-0149 §7  ·  impressão digital do pré-registo: `b3cae04eeace`
...
| # | o quê | valor |
|---|---|---|
| 1 | população usada (de 10542 linhas lidas) | **9966** em 16 clusters |
| 2 | selecionados / resto no limiar congelado | 325 / 9641 |
| 3 | média do desfecho: selecionados / resto | -0.0009 / -0.0006 |
| 4 | **D = média(selecionados) − média(resto)** | **-0.0003** |
| 5 | IC 95 % de D (bootstrap de cluster por `mercado`) | [-0.0026, +0.0017]  P(D≤0) = 0.615 |
| 6 | IC 95 % de D (bootstrap de blocos) | [-0.0031, +0.0021] em 18 blocos |
| 7 | p de permutação (estratificada por `dia`) | 0.8626 |
| 8 | fatia de teste reservada: D | +0.0004 (113/2973; 576 linhas purgadas) |
...
- D = -0.0003 não é positivo
- D = -0.0003 abaixo do MRE +0.0010
- IC 95 % inferior -0.0026 não está acima de zero
- p de permutação 0.8626 não é < 0,05
- o braço selecionado perde em nível (-0.0009); perder menos que o resto não é vantagem
- há dependência temporal declarada e o IC 95 % por blocos [-0.0031, +0.0021] cobre zero
...
|---|---|---|---|
| -0 | 9641 | -0.0006 | -0.0013 |
| 0-inf | 325 | -0.0009 | -0.0013 |
...
- 622.9 linhas por cluster: a permutação troca rótulos linha a linha e, com linhas dependentes dentro do cluster, o p sai otimista — leia o IC de cluster/blocos antes do p
