# H-003 — P6_breakout20 em h = 60 min (lado à vista, universo U2) — NÃO CONFIRMA

> Origem: R68 parte B — h=240 ficou inconclusivo em 5 células (partb_U2.txt 28–32: IC atravessando zero em todas) · KB-0149 §7  ·  impressão digital do pré-registo: `c7c82db08921`
...
| 2 | selecionados / resto no limiar congelado | 1042 / 19898 |
| 3 | média do desfecho: selecionados / resto | -0.0004 / -0.0011 |
| 4 | **D = média(selecionados) − média(resto)** | **+0.0007** |
| 5 | IC 95 % de D (bootstrap de cluster por `mercado`) | [-0.0000, +0.0014]  P(D≤0) = 0.031 |
| 6 | IC 95 % de D (bootstrap de blocos) | [-0.0007, +0.0019] em 19 blocos |
| 7 | p de permutação (estratificada por `dia`) | 0.8824 |
| 8 | fatia de teste reservada: D | -0.0006 (327/5861; 1152 linhas purgadas) |
...
- D = +0.0007 abaixo do MRE +0.0010
- IC 95 % inferior -0.0000 não está acima de zero
- p de permutação 0.8824 não é < 0,05
- o braço selecionado perde em nível (-0.0004); perder menos que o resto não é vantagem
- há dependência temporal declarada e o IC 95 % por blocos [-0.0007, +0.0019] cobre zero
- a fatia de teste não repete o sinal (D = -0.0006)
...
|---|---|---|---|
| -0 | 19898 | -0.0011 | -0.0014 |
| 0-inf | 1042 | -0.0004 | -0.0018 |
