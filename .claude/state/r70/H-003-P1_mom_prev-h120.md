# H-003 — P1_mom_prev em h = 120 min (lado à vista, universo U2) — REFUTA

> Origem: R68 parte B — h=240 ficou inconclusivo em 5 células (partb_U2.txt 28–32: IC atravessando zero em todas) · KB-0149 §7  ·  impressão digital do pré-registo: `0733f96bdd83`
...
| 2 | selecionados / resto no limiar congelado | 5011 / 4955 |
| 3 | média do desfecho: selecionados / resto | -0.0010 / -0.0002 |
| 4 | **D = média(selecionados) − média(resto)** | **-0.0008** |
| 5 | IC 95 % de D (bootstrap de cluster por `mercado`) | [-0.0014, -0.0001]  P(D≤0) = 0.988 |
| 6 | IC 95 % de D (bootstrap de blocos) | [-0.0020, +0.0002] em 18 blocos |
| 7 | p de permutação (estratificada por `dia`) | 0.5625 |
| 8 | fatia de teste reservada: D | -0.0020 (1525/1561; 576 linhas purgadas) |

Censura: 0 linhas sem desfecho, 0 sem a variável, 0 recusadas pela guarda anti-antecipação. Ausente nunca virou zero.

## VEREDITO: REFUTA

- IC 95 % superior +0.0002 abaixo do MRE +0.0010: evidência CONTRA uma vantagem útil desse tamanho — não é refutação de qualquer efeito, é refutação de um efeito que pague a mesa.
...
|---|---|---|---|
| -0 | 4955 | -0.0002 | -0.0009 |
| 0-1 | 5011 | -0.0010 | -0.0017 |
