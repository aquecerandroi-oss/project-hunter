# H-003 — P2_revert_z em h = 240 min (lado à vista, universo U2) — NÃO CONFIRMA

> Origem: R68 parte B — h=240 ficou inconclusivo em 5 células (partb_U2.txt 28–32: IC atravessando zero em todas) · KB-0149 §7  ·  impressão digital do pré-registo: `688860a8b2f9`
...
| 2 | selecionados / resto no limiar congelado | 133 / 4346 |
| 3 | média do desfecho: selecionados / resto | -0.0015 / +0.0005 |
| 4 | **D = média(selecionados) − média(resto)** | **-0.0019** |
| 5 | IC 95 % de D (bootstrap de cluster por `mercado`) | [-0.0051, +0.0015]  P(D≤0) = 0.874 |
| 6 | IC 95 % de D (bootstrap de blocos) | [-0.0083, +0.0033] em 17 blocos |
| 7 | p de permutação (estratificada por `dia`) | 0.5387 |
| 8 | fatia de teste reservada: D | -0.0059 (32/1503; 288 linhas purgadas) |
...
- D = -0.0019 não é positivo
- D = -0.0019 abaixo do MRE +0.0010
- IC 95 % inferior -0.0051 não está acima de zero
- p de permutação 0.5387 não é < 0,05
- o braço selecionado perde em nível (-0.0015); perder menos que o resto não é vantagem
- há dependência temporal declarada e o IC 95 % por blocos [-0.0083, +0.0033] cobre zero
- a fatia de teste não repete o sinal (D = -0.0059)
...
|---|---|---|---|
| -0 | 4346 | +0.0005 | -0.0012 |
| 0-inf | 133 | -0.0015 | +0.0001 |
