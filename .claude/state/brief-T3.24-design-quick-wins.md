...
- Tela real: product-designer abre Radar, Carteira, System, Markets nos dois temas (spec `tests/e2e/design-audit.config.ts`) e confere que nenhum badge/botão/campo mede < 4.5 (texto) / < 3 (contorno) no `metrics-*.json`. **Pré-condição (T3.24d, 2026-09-08):** o sign-up de teste está bloqueado pela instância Clerk (exige username + senha; relatório E3, `notes-T3.24d.md` §3) — até o Everton ajustar a instância, o revisor confere o diff + `theme-contrast.test.ts` e registra a captura como pendente, não como feita. A `ever` local já tem carteira principal (E2 resolvido), então a Carteira renderiza com dado real assim que houver sessão.
