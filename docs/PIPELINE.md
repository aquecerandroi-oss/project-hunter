...
9. **A resposta é a que o pipeline já tem.** Cada minuto preenchido vira `market.candles.closed` pela outbox, na mesma transação que gravou a vela (`upsert_candles`), exatamente como um minuto ao vivo. Observabilidade: `market_backfill_requests_total{outcome}` (`accepted`, `partial`, `truncated`, `empty`, `refused`, `ignored`, `duplicate`, `malformed`, `failed`), `market_backfill_minutes_total` e o log `market_backfill_planned` com mercado, shard, intervalo pedido/efetivo, minutos, pedaços, adiados e sem partição.
