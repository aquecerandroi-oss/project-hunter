...
| `MEME_MINT_COOLDOWN_AFTER_LOSS_S` | `300` | **decisão do dono** (T4.78) | check 28: segundos sem compra nova num mint cuja última posição real fechou com perda; `0` desliga. Opcional (ausente ⇒ 300); ilegível ou negativo recusa o boot pelo nome. **Evidência e ressalva, ambas escritas:** papel 19/09 = 19 recompras depois de saída por trailing, 0 alvos, −0,338 SOL (R64 §4); real: Musepaid (`operator/6` saiu 17:28:15 UTC, `operator/5` recomprou 17:28:40, perda); **contraexemplo** NARKY#2 (+0,0142 SOL) teria sido barrado — a Astra manteria a regra só em papel (parecer de 20/09, item 2); o Everton escolheu aplicá-la como **limite de exposição repetida**, não como vantagem demonstrada. Todas as recompras observadas foram < 5 min, daí 300 s e não 900. Medição em sombra: `meme_live_orders.reason = mint_cooldown_after_loss:*` (só quando tudo o mais passou), `refusals.mint_cooldown_after_loss` no heartbeat, `policy.mint_cooldown_after_loss_s`; regra de decisão em 3 dias na EXP-M21 |
...
| 27 | `launch_open_cap` | **só no perfil de lançamento** (§18.3): posições `lane = launch` + intenções de lançamento pendentes ≥ `MEME_LAUNCH_MAX_OPEN` | `launch_max_open_reached` |
| 28 | `mint_cooldown_after_loss` | **T4.78** (decisão do Everton, 20/09/2026): a última posição **real** fechada neste mint teve `pnl_sol < 0` há menos de `MEME_MINT_COOLDOWN_AFTER_LOSS_S` segundos (300 por padrão; `0` desliga). O insumo é `MemeWalletState.recent_losses` (`mint → exit_at` da perda mais recente, lido pelo executor de `meme_live_positions` **para o mint da proposta** — `mint = :mint AND status='closed' AND pnl_sol < 0 AND exit_at >= now − janela`, uma linha; nunca um "últimos N mints" global, que numa janela saturada omitiria justamente o candidato e leria como "sem perda" (revisão da Astra); um fechamento positivo nunca entra). Vale para **todos** os conjuntos da mesa (`operator/5`, `/6`) **e** para a pista de lançamento; **nunca** para vendas (§6). Roda **por último** nos dois perfis, de propósito: uma ordem recusada com este `reason` passou em tudo o mais — é a contagem que a medição em sombra da EXP-M21 precisa. Um `exit_at` no futuro (dois relógios discordando) recusa pela janela inteira, no máximo | `mint_cooldown_after_loss:<segundos_restantes>` (arredondado **para cima**) |
