...
| `MEME_TRADES_CONCURRENCY` | não | `8` | T4.2e: pulls da fita em voo ao mesmo tempo, atrás do bucket de `MEME_SWAP_API_BUDGET_60S` (que é o que os ritma). Sequencial (`1`), uma volta de 250 mints levava ~90 s e os tiers de "10 s" eram lidos a cada ~90 s — a causa dos 109/250 sem fita; com 8, ~10 s. Subir além de 8 não aumenta o orçamento, só a rajada |
