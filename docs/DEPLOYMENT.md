...
| `MEME_ACTIVITY_ENABLED` | não | `true` (com `MEME_SWAP_API_ENABLED`) | T4.2g: a fita por lote — `POST /v1/coins/market-activity/batch`, 50 moedas por requisição, uma volta por minuto sobre todos os rastreados na curva (`activity.py`) → `meme_market_activity_1m` e as colunas de fita de `meme_features_1m`/`meme_features_15s` quando a fita por mint não cobriu (`tape_source = activity_1m`). Gasta o **mesmo** orçamento do Cloudflare da fita: `ceil(rastreados/50)` requisições/min reservadas do `MEME_SWAP_API_BUDGET_60S` (3 de 16 com ~130). Desligado (ou com o swap-api desligado) diz `activity: disabled` no readiness e `enabled=false` na fonte `swap_api_activity` |
| `MEME_ACTIVITY_CYCLE_S` | não | `60` (mínimo 15) | cadência do lote. 60 = uma volta por minuto (o brief); 30 daria à porta de 15 s uma janela com ≤ 30 s de atraso ao custo de 6 requisições/min do mesmo orçamento — decisão do operador, declarada |
| `MEME_ACTIVITY_LEAD_S` | não | `3` (0–30) | quantos segundos **antes** de cada instante da grade o laço dispara, para a janela `1m` da resposta terminar logo antes do fecho do minuto que o fold julga (`tape_as_of` guarda o fim exato) |
...
| `MEME_FAST_LANE_COMMITMENT` | não | `confirmed` | T4.42: o `commitment` (`confirmed` \| `finalized`) que a via rápida de 15 s (`fast_lane.py`) usa para ler a curva. `confirmed` devolve ~11-12 s de atraso estrutural medido no KB-0117 — só alimenta proposta/marca de **papel**; o admissor real relê a curva ao vivo antes de qualquer ordem, então uma reorg de `confirmed` (rara) custa uma linha mal precificada, nunca dinheiro. O laço de minuto que marca `meme_curve_snapshots` para o PnL de papel continua em `finalized`, sem depender desta variável. Um valor desconhecido vira o padrão e um aviso, nunca uma queda; aparece no `hb:meme:radar` como `fast_lane_commitment` |
