...
**Redação por valor, não só por nome de chave (T4.62).** `redact_processor` (`hunter_core/logging.py`) só mascara pelo *nome* do campo — uma URL com `?api-key=...` embutida dentro de uma string livre (`event`, `exception` já renderizada por `format_exc_info`, ou `error=str(exc)`) passava batido, e foi assim que uma linha `HTTP Request: POST https://…?api-key=…` do próprio `httpx` (nível INFO, sem handler dedicado) vazava a chave. A correção tem três partes: (1) `redact_secret_values_processor`, último processador da cadeia (depois de `format_exc_info`), varre recursivamente todo campo string (`event`, `exception`, `error`, dicts/listas aninhadas) por `api-key=`, `apikey=`, `api_key=`, `token=`, `key=`, `secret=` e `password=`, mascarando o valor e preservando só os 4 primeiros caracteres; (2) `configure_logging` sobe `httpx`/`httpcore` para `WARNING` — o nível INFO deles nunca deveria ter existido em produção; (3) `redact_url()`, exportado por `hunter_core.logging`, é o mesmo mascaramento para adapters que montam sua própria mensagem de erro fora do pipeline de log (`hunter_exchanges.pumpfun.rpc_ws` ao logar uma falha de `websockets.connect`, cuja mensagem pode ecoar a URI com a chave). O event-gate do meme-worker ganhou, na mesma revisão, guardas por notificação (`event_gate_bad_frames`), um `try/except` em torno da escrita da lacuna de reconexão (`event_gate_gap_write_failed`) e `error_type`/traceback no log de reinício do gate (`event_gate_restarts_total`) — nenhuma falha pontual (frame malformado, erro de pool no `INSERT` da lacuna, exceção que matou o `TaskGroup`) deve mais silenciar métricas ou vazar segredo.
