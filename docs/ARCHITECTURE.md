...
- **`strategy-worker` também sharda por CPU, não só o `market-worker` por I/O (T3.74f).** Medido ao vivo: um único processo do worker de decisão fica preso em ~99 % de um núcleo durante uma rajada de ~200 mercados (fechamentos de timeframe compartilhados), enquanto o Postgres tem folga de sobra — o teto é o GIL de um processo Python, não a concorrência de I/O que `ShadowConfig.worker_concurrency` já resolve. `STRATEGY_SHARD=i/N` divide o universo monitorado entre N processos pela mesma fatia estável `crc32(symbol) % N` que `MARKET_SHARD` já usa (`hunter_core.sharding`, uma função, dois consumidores), cada shard com seu próprio grupo consumidor no stream inteiro e seu próprio `hb:strategy:shadow:{i}of{N}` — a API agrega o hop `decision` de `GET /api/v1/system/latency` pelo pior shard vivo, e a varredura genérica de `/api/v1/system/workers` mostra uma linha por shard sem mudança de código. Desenho completo em `docs/DEPLOYMENT.md` §3.1b e `docs/PIPELINE.md` §6b.
