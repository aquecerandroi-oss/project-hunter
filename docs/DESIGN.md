...
- **Densidade.** Grade de 4px, 8px entre grupos. Escala de **7 degraus nomeados** (DESIGN-5, 2026-09-08 -- a escala de 5 + exceções não descrevia mais a interface: `11px` aparecia em 36 lugares e `18px` em 16): `11` **micro** (metadado de uma linha: idade, código de exchange, cobertura, hint de relógio, `kbd`), `12` labels/eyebrows, `13` corpo de **toda** tabela de dados (Markets, Radar, Lab, Carteira, System -- era exceção só de `markets-table.tsx`, virou regra porque todas as tabelas já a seguiam), `14` texto, `16` texto de destaque, `20` títulos de seção e valor de stat/card, `24` KPI em grade (vários por tela, `tabular-nums` mono), `28` número-herói (um por tela: o preço no detalhe do mercado). **Proibidos:** `10px` (única exceção viva: a dica `Ctrl K` do command palette, `command-palette.tsx`) e `18px` (o `/_design` dev-only migra também). Tabelas com **40px** de altura de linha no modo confortável e **32px** no compacto (`hooks/useDensity.ts`'s `useRowHeight()` -- a mesma constante alimenta a virtualização E a altura inline de cada linha, nunca dois números que podem divergir). Um componente novo escolhe um dos 7 degraus; não existe "meio degrau".
- **Badges e campos têm contraste medido nos dois temas** (DESIGN-5). Fundo de badge é sempre o token `-soft` da cor (`green-soft`, `red-soft`, `warning-soft`, `info-soft`, `gold-soft`), nunca `bg-x/15` composto por alfa -- a composição media 3.93:1 (claro, verde e âmbar) e 4.34:1 (escuro, vermelho). Texto sobre `red` (botão destrutivo) é `bg` (`#0A0A0A` no escuro, `#FFFFFF` no claro), nunca `white` fixo (3.76:1 no escuro). Item desabilitado/planejado usa `fg-subtle` **sem** `opacity` (a opacidade derrubava para 2.4:1). O contorno de um campo de formulário (`input`, `select`, checkbox) usa `border-input` e mede **>= 3:1** contra o fundo do campo e contra o fundo do card (WCAG 1.4.11) -- `border`/`border-strong` são decorativos (linhas de tabela, separadores) e ficam em ~1.3:1 de propósito. Os pares entram em `apps/web/tests/theme-contrast.test.ts`.
- **Vocabulário de tempo, três rótulos e só três** (DESIGN-5): **"Consultado em dd/mm/aaaa hh:mm:ss"** (quando o servidor leu -- `as_of` de página/seção), **"Atualizado há N s | N min | N h"** (idade de um dado que envelhece: ticker, heartbeat, MTM), **"Snapshot · há N s"** (foto tirada no load: book/trades). Sempre Brasília no texto (T3.22) e o ISO UTC no `title`. Um único componente (`components/time/brasilia-instant.tsx`); nada de "Painel consultado", "Estado em", "consultado em" minúsculo ou idades sem espaço (`12s`).
- **Sem backstage na copy** (DESIGN-5): nenhum id de tarefa (`T3.18`), número de ADR, caminho de arquivo, nome de serviço interno ou comando de terminal aparece na interface. O "não construído ainda" nomeia o **milestone** ("chega no M3"); o operador acha o comando no runbook (`docs/ACTIVATION.md`), que a tela pode linkar ou esconder atrás de `<details>` "Para o operador". Valores de enum nunca chegam crus à tela (`ENTRY_CANDIDATE`, `alive`, `long`, `paper`): cada domínio tem um dicionário plain-data (`components/<dominio>/labels.ts`) coberto por teste que falha quando um membro novo do enum não tem rótulo.
...
- 2026-09-08 — DESIGN-5 (primeira auditoria do product-designer, T3.23, `.claude/state/review-design-2026-09-08.md`): (a) **contraste medido a partir dos valores exatos dos tokens, incluindo as composições com alfa** (`.claude/state/design/2026-09-08/contrast-tokens.md`, 29 pares × 2 temas) achou 7 falhas AA em componentes compartilhados que os testes não cobriam: badge `negative` escuro 4.34:1 (`red` sobre `red/15`), badges `positive`/`warning` claros 3.93:1, badge `gold` claro 4.49:1, botão destrutivo escuro 3.76:1 (`white` sobre `red`), item "Planejado" da sidebar 2.4:1 nos dois temas (`opacity-60`), contorno dos campos 1.26:1 e campo sobre card 1.04:1. Regras novas acima em §2 ("Badges e campos têm contraste medido"); valores decididos, **a aplicar em `globals.css` pelo brief T3.24a** (a tabela §1 muda junto com o código): tema claro `--color-gold` `#8A6D00` → `#7F6400` (5.14:1 sobre `gold-soft`, 5.64:1 sobre branco e como fundo de `gold-fg`), `--color-gold-strong` → `#665000`, `--color-warning` `#B45309` → `#A34A05`; tokens novos `--color-warning-soft` (`#2E1F06` / `#FEF3C7`), `--color-info-soft` (`#0F1F33` / `#DBEAFE`), `--color-border-input` (`#666666` / `#8A8A8A`, 3.15:1 / 3.11:1 sobre `bg-overlay`); badges passam a `-soft`; destrutivo `text-bg`; planejado sem `opacity`. (b) **Escala tipográfica** reescrita como 7 degraus (11/12/13/14/16/20/24/28), `10px` e `18px` proibidos, `13px` generalizado para toda tabela de dados -- a interface já era assim; a doc é que mentia. (c) **Vocabulário de tempo** (três rótulos) e **sem backstage na copy** (ids de tarefa, ADR, caminhos, enums crus) -- regras que faltavam e que a auditoria achou quebradas em 5 e 20+ lugares respectivamente. Decisões abertas ao Everton (idioma dos nomes de página, convenção numérica en-US vs pt-BR -- §2 diz `+1,23%` e o código imprime `+1.23%` -- e a hierarquia do Lab) estão no relatório, não aqui.
