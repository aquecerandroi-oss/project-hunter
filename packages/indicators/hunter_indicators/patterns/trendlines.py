"""The lines themselves: two pivots define a candidate, the tape decides.

The hand-drawn rule, made deterministic. A candidate is the straight line
through **two same-side pivots** (highs give resistance, lows give support).
It becomes a *line* when the tape has honoured it:

- **touches** — at least ``min_touches`` (3, the classic "two points draw it,
  the third confirms it") same-side pivots sit within ``tolerance_atr`` of it;
- **respected** — between the first and the last touch, no close is beyond it
  by more than ``tolerance_atr``. One close through the line is a wick; two are
  a different line;
- **score** ``touches × span − violations``, where ``span`` is the distance in
  bars between the first and last touch and ``violations`` counts the closes
  through the line **after** the first touch up to the cut. So a line that held
  for 200 bars beats one that held for 20, and a line the market has already
  left ranks below one it is still respecting — while staying in the output,
  because the breakout of a line nobody can see is not a breakout.

Two anti-look-ahead facts are structural, not checks bolted on: only pivots
whose ``confirmed_at`` has already happened can anchor or touch a line, and
...
consumer may act on the line. The geometry of a line is history; its
*existence* has a date.

What this module does **not** promise: that the same line would have been
*selected* at ``valid_from_idx``. Scores, buckets and ``max_lines`` are
evaluated at the cut, so a scan at bar 500 is a retrospective drawing of the
lines that survive **today**, not a replay of what the top six were at bar 300.
A consumer that needs the live sequence calls :func:`scan` once per bar with
``as_of`` on that bar — which is exactly what a strategy does.
"""
...
from hunter_indicators.patterns.pivots import Pivot, PivotKind
...
class TrendLine:
    ...
    just passed (measured: 0.485 ATR of drift and one such line on the real
    14-day series).
    """
    through: tuple[int, Decimal]
    """The second pivot of the pair. With ``origin`` it *is* the line.

    Kept next to ``origin`` because ``valid_from_idx`` has to be at or after the
    confirmation of **both** (Astra, T3.34 review, must-fix 2), and a consumer
    that wants to prove that reads it here instead of trusting the field.
    """
...
def _build(
    ...
    # A line exists when the tape has confirmed BOTH the pivots that fix its
    # slope and the `min_touches`-th touch. Taking only the touch (Astra, T3.34
    # review, must-fix 2) let a line whose slope came from pivots at bars 310
    # and 323 claim to have been valid since bar 232 — and every event searched
    # from that bar was then measured against a slope from the future.
    valid_from = max(
...
def _rank(line: TrendLine) -> tuple[Decimal, int, int, int, int, str]:
    ...
    synthetic wave does exactly that — and then the only honest preference is
    the one that could have been drawn **earlier**. Falling through to
    ``line_id`` would pick by hash, which is deterministic and meaningless.
    """
    return (
        -line.score,
        -line.touches,
...
def _dedupe(
    ...
    are one; two lines he would have drawn separately keep their identity.
    """
