...
class _staged_at:
    """Downgrade to ``revision`` for the body, always come back to head (T4.21)."""

    def __init__(self, upgraded: str, revision: str) -> None:
        self.config = alembic_config(upgraded)
        self.revision = revision

    def __enter__(self) -> Config:
        command.downgrade(self.config, self.revision)
        return self.config
...
def test_0033_seeds_operator_3_through_the_flow_gate_and_retires_operator_2(
    ...
    with _staged_at(upgraded, OPERATOR_3_REVISION):
        seeds = asyncio.run(
            _scalars(
                upgraded,
                "SELECT name || '/' || version || ':' || kind || ':' || coalesce(exp_ref, '-') || ':' "
...
def test_0033_refuses_a_downgrade_while_a_proposal_or_a_bet_references_operator_3(
    ...
    with _staged_at(upgraded, OPERATOR_3_REVISION) as config:
        ...
        for statements, message in guarded:
            ...
            try:
                with pytest.raises(DBAPIError, match=message):
                    command.downgrade(config, "-1")
                assert asyncio.run(_revision(upgraded)) == OPERATOR_3_REVISION, (
                    "the downgrade must not commit"
                )
            finally:
...
_FLOW_V2_ARM2_RULE_SET = "01994d00-6c1a-7000-8000-00000000000b"
_OPERATOR_4_RULE_SET = "01994d00-6c1a-7000-8000-00000000000c"
_ARM2_KEYS = (
    "'gate_version' - 'max_snipers' - 'min_holders' - 'holders_rising_or_flat' "
    "- 'creator_unknown_allowed_if_dev_measured' - 'progress_or_mcap_rising'"
)
_CLEAN_0034: tuple[tuple[str, dict[str, object]], ...] = (
    ("DELETE FROM meme_paper_bets WHERE mint = 'GUARD_MINT'", {}),
    ("DELETE FROM meme_proposals WHERE mint = 'GUARD_MINT'", {}),
)


def test_0034_seeds_the_second_arm_and_hands_the_desk_to_operator_4(upgraded: str) -> None:
...
def _refusal_of_0034(upgraded: str, config: Config) -> None:
    ...
    guarded: list[tuple[list[tuple[str, dict[str, object]]], str]] = [
        (
            [(_A_PROPOSAL, {"id": proposal, "rule_set": _OPERATOR_4_RULE_SET})],
            "proposals reference the seeded flow_v2/2 or operator/4",
        ),
        (
            [
                (_A_PROPOSAL, {"id": proposal, "rule_set": _RESEARCH_RULE_SET}),
                (
                    _A_BET,
                    {
                        "id": bet,
                        "proposal": proposal,
                        "rule_set": _FLOW_V2_ARM2_RULE_SET,
                        "mode": "paper",
                    },
                ),
            ],
            "bets reference the seeded flow_v2/2 or operator/4",
        ),
    ]
    for statements, message in guarded:
        asyncio.run(_write(upgraded, statements))
...
def test_0034_reverses_on_a_clean_database_and_comes_back(upgraded: str) -> None:
    """The operator's rollback: both arm-2 sets go, ``operator/3`` comes back
    active, ``flow_v2/1`` untouched; the upgrade restores the hand-over."""
    config = alembic_config(upgraded)
    command.downgrade(config, OPERATOR_3_REVISION)
    try:
        assert asyncio.run(_revision(upgraded)) == OPERATOR_3_REVISION
        assert asyncio.run(
            _scalars(
                upgraded,
                "SELECT count(*)::text FROM meme_rule_sets WHERE id IN (:a, :o)",
                {"a": _FLOW_V2_ARM2_RULE_SET, "o": _OPERATOR_4_RULE_SET},
            )
        ) == ["0"]
        assert asyncio.run(_scalars(upgraded, _ACTIVE_OPERATOR_SETS, {})) == ["operator/3"]
        assert asyncio.run(
            _scalars(
                upgraded,
                "SELECT status FROM meme_rule_sets WHERE id = :f",
                {"f": _FLOW_V2_RULE_SET},
            )
        ) == ["active"]
    finally:
...
def test_0034_refuses_to_upgrade_a_desk_that_already_has_another_active_operator_set(
    upgraded: str,
) -> None:
    """The desk's invariant, asserted after the hand-over (as ``0033``): a
    second active ``operator`` set makes the upgrade stop and commit nothing."""
    config = alembic_config(upgraded)
    extra = "00000000-0000-4000-8000-000000000b09"
    command.downgrade(config, OPERATOR_3_REVISION)
    try:
        asyncio.run(_write(upgraded, [(_AN_EXTRA_OPERATOR_SET, {"id": extra})]))
        try:
            ...
            assert asyncio.run(_scalars(upgraded, _ACTIVE_OPERATOR_SETS, {})) == [
                "operator_guard_0033/1",
                "operator/3",
            ], "the refused upgrade left operator/3 active and planted nothing"
        finally:
            asyncio.run(
                _write(upgraded, [("DELETE FROM meme_rule_sets WHERE id = :id", {"id": extra})])
            )
...
def test_0042_refuses_a_downgrade_while_a_row_carries_the_cap(upgraded: str) -> None:
    ...
    # T4.26b: staged at 0042 since 0043 sits on top (``"-1"`` would reverse 0043).
    with _staged_at(upgraded, EXECUTABLE_MCAP_REVISION) as config:
        asyncio.run(
            _write(
                upgraded,
                [
        ...
        try:
            with pytest.raises(DBAPIError, match="carry an executable market cap"):
                command.downgrade(config, "-1")
            assert asyncio.run(_revision(upgraded)) == EXECUTABLE_MCAP_REVISION, (
                "the downgrade must not commit"
            )
        finally:
            asyncio.run(_write(upgraded, list(_CLEAN_0042)))
...
# ---------------------------------------------------------------------------
# 0043_meme_events_scan_cursor — an event may now name many coins (T4.26b)
# ---------------------------------------------------------------------------

_CLEAN_0043: tuple[tuple[str, dict[str, object]], ...] = (
    ("DELETE FROM meme_event_matches WHERE mint LIKE 'X43_%'", {}),
    ("DELETE FROM meme_events WHERE title LIKE 'T4.26b test%'", {}),
    _RETENTION_MARKER,
    ("DELETE FROM meme_tokens WHERE mint LIKE 'X43_%'", {}),
)


async def _write_returning(url: str, sql: str, params: dict[str, object]) -> list[str]:
    """Like ``_write``, but for a single ``RETURNING`` statement whose result
    must be read back — committed, unlike ``_scalars``' bare connection."""
    engine = async_engine(url)
    try:
        async with engine.begin() as connection:
            result = await connection.execute(text(sql), params)
...
def _insert_test_event(event_id: str) -> tuple[str, dict[str, object]]:
    return (
        "INSERT INTO meme_events (id, observed_at, source, kind, title, symbol_hint, "
        "confidence, recorded_by) VALUES (:id, now(), 'manual', 'incident', "
        "'T4.26b test event', 'X43', 'rumor', 'test')",
        {"id": event_id},
    )


def test_0043_adds_the_scan_cursor_column(upgraded: str) -> None:
    present = asyncio.run(
        _scalars(
            upgraded,
            "SELECT column_name FROM information_schema.columns "
            "WHERE table_name = 'meme_events' AND column_name = 'last_scanned_created_at'",
            {},
        )
    )
    assert present == ["last_scanned_created_at"]
    command.check(alembic_config(upgraded))


def test_0043_creates_meme_event_matches_with_its_grants_and_fks(upgraded: str) -> None:
    columns = asyncio.run(
        _scalars(
            upgraded,
            "SELECT column_name FROM information_schema.columns "
            "WHERE table_name = 'meme_event_matches' ORDER BY column_name",
            {},
        )
    )
    assert set(columns) == {"event_id", "mint", "match_kind", "matched_at"}
    assert asyncio.run(_table_privileges(upgraded, "hunter_app", "meme_event_matches")) == {
        "SELECT"
    }
    assert asyncio.run(_table_privileges(upgraded, "hunter_worker", "meme_event_matches")) == {
        "SELECT",
        "INSERT",
    }
    command.check(alembic_config(upgraded))
...
def test_0043_a_pair_matched_once_stays_matched(upgraded: str) -> None:
    """T4.26b's own idempotency rule: ``ON CONFLICT (event_id, mint) DO
    NOTHING`` — a second insert of the same pair changes nothing, and a
    different ``match_kind`` on the retry does not overwrite the first."""
    event_id = "00000000-0000-4000-8000-000000004301"
    asyncio.run(
        _write(
            upgraded,
            [(_A_TOKEN, {"mint": "X43_PAIR"}), _insert_test_event(event_id)],
        )
    )
    try:
        insert = (
            ...
            "ON CONFLICT (event_id, mint) DO NOTHING RETURNING mint"
        )
        first = asyncio.run(
            _write_returning(
                upgraded, insert, {"event_id": event_id, "mint": "X43_PAIR", "match_kind": "buy"}
            )
        )
        assert first == ["X43_PAIR"]
        second = asyncio.run(
            _write_returning(
                upgraded,
                insert,
                {"event_id": event_id, "mint": "X43_PAIR", "match_kind": "avoid"},
            )
        )
        assert second == [], "the pair is already matched; the retry inserts nothing"
        stored = asyncio.run(
            _scalars(
                upgraded,
                "SELECT match_kind FROM meme_event_matches WHERE event_id = :id AND mint = :mint",
                {"id": event_id, "mint": "X43_PAIR"},
            )
        )
...
def test_0043_check_refuses_an_unknown_match_kind(upgraded: str) -> None:
    event_id = "00000000-0000-4000-8000-000000004302"
    asyncio.run(
        _write(
            upgraded,
            [(_A_TOKEN, {"mint": "X43_BAD"}), _insert_test_event(event_id)],
        )
    )
    try:
        with pytest.raises(DBAPIError, match="match_kind_is_a_known_label"):
            asyncio.run(
                _write(
                    upgraded,
                    [
...
def test_0043_refuses_a_downgrade_while_a_match_exists(upgraded: str) -> None:
    ...
    asyncio.run(
        _write(
            upgraded,
            [
                (_A_TOKEN, {"mint": "X43_GUARD"}),
                _insert_test_event(event_id),
                (
                    "INSERT INTO meme_event_matches (event_id, mint, match_kind) "
                    "VALUES (:id, :mint, 'buy')",
                    {"id": event_id, "mint": "X43_GUARD"},
                ),
            ],
        )
    )
    try:
        ...
        asyncio.run(
            _write(
                upgraded,
                [("DELETE FROM meme_event_matches WHERE event_id = :id", {"id": event_id})],
            )
        )
        asyncio.run(
            _write(upgraded, [("DELETE FROM meme_events WHERE id = :id", {"id": event_id})])
...
def test_0043_reverses_on_a_clean_database_and_comes_back(upgraded: str) -> None:
    config = alembic_config(upgraded)
    command.downgrade(config, EXECUTABLE_MCAP_REVISION)
    try:
        assert asyncio.run(_revision(upgraded)) == EXECUTABLE_MCAP_REVISION
        assert not asyncio.run(_relation_exists(upgraded, "meme_event_matches"))
        assert asyncio.run(
            _scalars(
                upgraded,
                "SELECT count(*)::text FROM information_schema.columns "
