"""Trend-line primitives, part 3: what price does to a line, and the one cut.

Ported from ``hunter_indicators.patterns.events`` (T3.34, upstream commit ``db798b8``); :mod:`hunter_core.strategies.tl_pivots`
explains why the geometry is copied instead of imported, and
...
2. the first close beyond it by more than ``break_atr`` is the **breakout**. With
   ``rvol_min`` given, a break without relative volume is **not** an event: it is
   reported nowhere, because "it broke, quietly" is exactly the break that fails;
3. after a breakout, a bar that comes back within ``tolerance_atr`` of the line
   and closes again in the direction of the break, within ``retest_bars``, is the
   **retest**. Nothing after that: the line stops being a line once the market
   has left it, and a second break of the same geometry would be a new line drawn
   on new pivots.

Every event is decided **at the close of the bar it is stamped with** and never
before ``line.valid_from_idx``. A wick that goes *through* the line by more than
the tolerance and closes back is not a bounce here (it is a failed break, and
naming it would let a strategy buy a level that has just been pierced).

The cut itself is applied once, at the top of
:func:`hunter_core.strategies.tl_scan.tl_scan`, so everything here sees a window
that already **ends** at the bar being decided.
"""
...
from hunter_core.strategies.tl_lines import DEFAULT_TOLERANCE_ATR, LineKind, TrendLine
...
def _volume_confirms(
    index: int, rvol: Sequence[Decimal | None] | None, rvol_min: Decimal | None
) -> bool:
    if rvol_min is None:
        return True
...
def _bounce_confirmation(
    ...
    for index in range(touch_idx, min(touch_idx + bounce_bars, cut) + 1):
        ...
        with localcontext(CONTEXT):
            ...
            broke = past > break_atr * scale
            bounced = -past >= bounce_atr * scale
        if broke:
            return None
        if bounced:
            return index
    ...
    *,
    ...
    for line in lines:
        events, break_idx = _walk_until_break(
            bars, atr, line, cut, tolerance_atr, break_atr, bounce_atr, bounce_bars, rvol, rvol_min
        )
        found.extend(events)
