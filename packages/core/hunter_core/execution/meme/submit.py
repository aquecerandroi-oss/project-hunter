...
- **Re-send, never re-sign** (§9.4 rule 3, T4.55): while the confirmation loop
  waits (``confirm.py``), the *same signed bytes* go out again every
  ``resend_interval_s`` until the transaction lands, the blockhash's
  ``last_valid_block_height`` is passed (``blockhash_expired_never_landed``,
  decided there instead of 50 s later by reconciliation) or the window closes.
  R56 §2.1 measured 3 of ~23 real sends lost to a single unrepeated send.
...
- **Jito** (§9.3) is a parameter, off by default; the bundle carries one
  transaction and confirmation still goes through the signature (no re-send).
"""
...
from hunter_core.execution.meme.submit_types import (
    LANDED,
    ApprovedSubmission,
    BundleSender,
...
class MemeSubmitter:
    ...
    def _attempt(self, approval: ApprovedSubmission) -> SubmitResult:
        ...
        bundled = self._policy.jito_bundle and self._bundles is not None
        try:
            if bundled and self._bundles is not None:
        ...
        settled = poll_until_settled(
            ...
            self._policy,
            signature,
            resend=None if bundled else transaction,
            last_valid_block_height=approval.last_valid_block_height,
            sleep=self._sleep,
        ...
        if settled.outcome == "failed":
            return self._fail(pid, settled.reason, signature, settled.stats)
        return self._unconfirmed(pid, settled.reason, signature, settled.stats)

    # --------------------------------------------------------------- confirm
    def _settle_landed(
        self, pid: str, signature: str, stats: ResendStats | None = None
    ) -> SubmitResult:
        try:
            transaction = self._rpc.get_transaction(signature, commitment=self._policy.commitment)
        except Exception as exc:
            return self._unconfirmed(
                pid, f"rpc_unreachable_fetching_fill:{type(exc).__name__}", signature, stats
            )
        if transaction is None:
            return self._unconfirmed(
                pid, "transaction_not_found_after_confirmation", signature, stats
            )
        try:
            ...
            return self._unconfirmed(
                pid, f"fill_decode_failed:{type(exc).__name__}", signature, stats
            )
        if not fills:
            return self._unconfirmed(pid, "trade_event_missing", signature, stats)
        fill = fills[0]
        self._journal.record_state(pid, SubmitState.CONFIRMED, "trade_event", fill)
        return result_of(pid, SubmitState.CONFIRMED, "trade_event", signature, stats, fill)
    ...
    def _fail(
        ...
        stats: ResendStats | None = None,
    ) -> SubmitResult:
        if self._journal.get(pid) is None:
            self._journal.begin_signing(pid)
            self._journal.release_signing(pid)
        self._journal.record_state(pid, SubmitState.FAILED, reason, None)
        return result_of(pid, SubmitState.FAILED, reason, signature, stats)

    def _unconfirmed(
        self, pid: str, reason: str, signature: str, stats: ResendStats | None = None
    ) -> SubmitResult:
        self._journal.record_state(pid, SubmitState.SUBMITTED_UNCONFIRMED, reason, None)
        return result_of(pid, SubmitState.SUBMITTED_UNCONFIRMED, reason, signature, stats)
