...
> **Arquivada pela Sexta-feira em 2026-09-09 (T3.55b)** a partir do rascunho do `quant-engineer`
> (`.claude/state/exp-drafts/KB-0081-candlestick-no-nosso-dado.md`). **Nada foi commitado, nada de
> produção foi tocado**: nenhum arquivo de `packages/`, nenhum `code_ref`, nenhum
> `feature_set_version`. O protótipo mora em `.claude/state/exp-drafts/t355/` e não é importado por
...
> **Veredito em uma linha:** de 144 testes-rótulo (72 + 72, duas famílias de definição), **nada
> sobrevive a Holm**, exceto um — e esse um (marubozu de baixa em 1 h) prevê o **oposto** do que o
> padrão afirma, com o pedágio comendo a vantagem inteira quando operado ao contrário
> (+0,005 R, IC `[−0,104; +0,112]`). A geometria mata os padrões de uma barra antes de qualquer
> estatística: enforcado e doji não têm invalidação que caiba no pedágio.
...
com as **outras** definições (as 18 da [[KB-0080-candlestick-evidencia]] §7, constantes diferentes, contexto de tendência
...
6. **Bootstrap de blocos por dia** (`t342-blocos/blocos.py`, **reusado sem uma linha de alteração**),
   ...
   mercados vão juntos**, porque em 16 perpétuas correlacionadas o choque é do dia ([[KB-0010-overfitting-de-backtest-e-o-preco-de-cada-variante]]).
...
5. **Duas famílias, e a segunda escolhida depois de a primeira existir.** O Holm conjunto sobre 144
   ...
   e ela é enorme ([[KB-0080-candlestick-evidencia]] §2). O prior verdadeiro é pior que o meu p.
...
|---|---|---|---|
| C1 | Plausibilidade da vantagem | **FAIL** | O mecanismo declarado ("a vela mostra quem ganhou a barra") não sobreviveu ao teste na fonte: 144 testes, um sobrevivente, e com o **sinal invertido**. A literatura concorda ([[KB-0080-candlestick-evidencia]] §6: nada nas grandes em cripto diário; o único "sim" intradiário não foi lido). Não há hipótese a promover |
| C2 | Risco de sobreajuste | **FAIL se alguém insistir** | Foram medidos 32 rótulos em 2 timeframes e 3 horizontes com **duas** parametrizações. Escolher agora o `a_marubozu_baixa` invertido é escolher o máximo de 144 — exatamente o data snooping que [[KB-0003-rompimento-de-canal-e-data-snooping]] descreve. Só valeria com **janela nova** (out-of-sample de verdade) |
...
| C7 | Realismo de execução | **PASS na hipótese, FAIL no resultado** | Mesmos 20 bps ida-e-volta das outras versões, mesma identidade da [[KB-0076-por-que-perdemos-2026-09-08]] verificada. O resultado é que os padrões não cabem nela |
...
4. **Repetindo com as definições da [[KB-0080-candlestick-evidencia]] §7, o resultado se mantém**, com um único
...
10. **O que mudaria a resposta:** 6–12 meses de 1 h nos mesmos mercados, ou o artigo de Moser &
    Brauneis (2026) lido de verdade ([[KB-0080-candlestick-evidencia]] §2.7), que é a única evidência intradiária com
...
4. **A leitura invertida do marubozu** (`+0,005 R`) merece uma janela nova de teste ou é
   exatamente o tipo de coisa que a [[KB-0003-rompimento-de-canal-e-data-snooping]] manda descartar?
...
[[KB-0003-rompimento-de-canal-e-data-snooping]] · [[KB-0010-overfitting-de-backtest-e-o-preco-de-cada-variante]] (bloco de dia) ·
