...
> **Arquivada pela Sexta-feira em 2026-09-08 (T3.32b)** a partir do rascunho da T3.32
> (`.claude/state/exp-drafts/KB-0076-por-que-perdemos-2026-09-08.md`). Nada aqui é dinheiro real
...
positivo e tem **25 desfechos**: `inconclusivo` por contrato (`docs/plans/SHADOW-LAB.md` §9), e o
**mesmo v4** dá −0,1506 R no replay de 30. A coorte `replay:7598d6c4` é a corrida gêmea de
`f8d8279c` **antes** do backfill de funding (201 das 224 sem `R_net`): entra na tabela para mostrar
que 23 sobreviventes não são população, não como resultado.

O funding **não é a causa**: entre −0,0002 R e +0,0006 R por operação em todas as populações, mil
vezes menor que o custo de execução. [[KB-0019-o-que-a-nossa-funding-rate-mede-de-fato]] e
[[KB-0021-funding-como-preco-de-posicionamento-nao-como-previsao]] ficam confirmadas na direção
"funding é pequeno nesta janela", não refutadas.
...
- **Regime** (`2026-09-08-07`): **`market_regimes` tem uma única linha, `UNKNOWN`**, e os 3 040
  sinais com `regime_id` apontam todos para ela. O classificador de `docs/PIPELINE.md` §4 nunca
  produziu série — hoje **não existe** filtro de regime para ligar. Reconstruindo do BTC 1 h,
...
### 6. A invalidação: a [[KB-0006-invalidacao-stop-por-atr-ou-saida-por-tempo]] acerta a descrição e erra o remédio
...
Remédio testado ([[EXP-0007-momentum-invalidacao-bracos-INV]], replay de braços sobre as **mesmas**
entradas congeladas): **remover a invalidação não muda nada.** `INV-B − base` = −0,006 R (replay
`momentum v2`, 222 pares), +0,032 R (`volume_anomaly v2`, 337), −0,070 R (`momentum v2`
prospective), −0,038 R (`momentum v1` prospective, 932 pares). Nenhum passa o efeito mínimo
declarado de 0,05 R nem o Holm, e dois dos quatro têm sinal negativo. Os trades que a invalidação
mata **iriam perder de qualquer forma**: ela adianta a perda, não a cria.
...
**Duas populações, sinais opostos.** É a
[[KB-0010-overfitting-de-backtest-e-o-preco-de-cada-variante]] em estado puro: a janela
2026-08-08→09-08 sobre quatro majors gerou a hipótese e a população viva a contradiz. A distribuição
de MFE sustenta a dúvida em vez de resolvê-la: entre os ganhadores de `momentum v2` o MFE p90 é
1,23 R (replay) e 1,20 R (prospective) — **não há cauda direita grossa** que um alvo mais distante
capturasse nesta janela — e entre os perdedores o MAE p50 é 0,74 R, isto é, o stop é tocado de
raspão.
...
abaixo dos 30 dias e/ou dos 100 desfechos que o `docs/plans/SHADOW-LAB.md` §9 exige. **Veredito de
toda variante individual: `inconclusivo`.** O que **não** é inconclusivo é a identidade do custo:
ela é aritmética, não estatística, e vale para qualquer n.
...
1. **Orçamento de custo explícito na decisão.** Uma versão que recuse decidir quando
   ...
   **Aviso registrado antes do teste** (`2026-09-08-08-piso-de-risco.sql`, contrafactual *dentro da
   ...
   já está medindo desde 2026-09-08 13:05Z ([[EXP-0006-momentum-piso-de-custo]]).
2. **Existe alguma entrada com bruto > 0,25 R?** É o teste que falta. Nenhuma das dez populações
   ...
   — a [[KB-0008-custos-em-perpetuos-e-o-r-que-sobra]] já dizia isso e agora tem o número desta
   base. **É a pergunta que abriu as quatro candidatas novas**
   ([[EXP-0008-breakout-compressao-de-volatilidade]],
...
- **A hipótese de custo pode estar pessimista.** 20 bps de ida e volta é hipótese declarada do Lab,
  não tarifa medida ([[KB-0008-custos-em-perpetuos-e-o-r-que-sobra]]). Se o custo real for metade, a
  expectancy líquida melhora exatamente `custo_R/2` e continua negativa em 8 das 10 populações,
  porque o bruto é ~0. Medir o custo real (execução paper contra o book) mudaria a **magnitude**,
  não o sinal.
- **`volume_anomaly` pode estar operando exaustão como se fosse continuação.** Bruto positivo
  (+0,009 a +0,020 R) com acerto de 27–33 % e alvo a 1,5 ATR é compatível com
  [[KB-0015-volume-relativo-e-o-pico-como-exaustao]]; esta nota **não** testou direção invertida e
  não deve ser lida como se tivesse testado.
- **A elegibilidade do replay é a de hoje**, não a da janela (`docs/PIPELINE.md` §6c): ETH/SOL/XRP/
  DOGE é escolha da corrida, e as populações prospectivas correm sobre um universo muito maior e
  mais volátil (risco mediano 1,44 % contra 0,79 %). As duas não são comparáveis entrada a entrada —
  foi por isso que os contrastes foram computados **dentro** de cada uma.
...
**Pendente.** A Astra revisou, no mesmo dia, as **quatro estratégias novas**
([[Dialogos/2026-09-08-quatro-estrategias]]) e não esta nota. O que dela já vale aqui, e está
absorvido: o custo assumido de 20 bps é hipótese, não tarifa spot verificada, e uma leitura positiva
a 20 bps que vira negativa a 40 bps é **"frágil a custos"**, não validação. Enquanto o parecer sobre
esta nota não existir, o campo `astra:` fica `pendente` de propósito.
...
- Rascunho de origem: `.claude/state/notes-T3.32.md` e
  `.claude/state/exp-drafts/KB-0076-por-que-perdemos-2026-09-08.md`.
