...
| `has_twitter`, `twitter_kind`, `twitter_post_age_s`, `twitter_reuse_count`, `has_website`, `has_telegram`, `description_len` | `identity` (T4.26, EXP-M8) | identidade social do token | T4.26 | só quando `require_twitter` | não | — |
...
5. `has_twitter`/`twitter_kind`/`twitter_reuse_count` (meme, `identity`) — coletado desde T4.26, nunca virou hipótese.
