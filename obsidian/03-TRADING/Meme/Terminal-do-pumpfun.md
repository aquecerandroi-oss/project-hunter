...
| Card de moeda | ticker + nome, `MC` (mcap US$), `V` (volume), `F` (taxas em SOL), idade, holders, nº de transações `x/y`, **top-10 %**, **dev %**, snipers %, sociais (@X, links), os selos "Boost"/"Paid" (são produtos do DEX Screener, não do pump.fun — perfil pago ≠ pedido pago ≠ boost; [[KB-0093-dex-paid-e-boost-o-que-custam-e-o-que-medem]]) e o botão **"0 SOL"** (compra rápida com o preset — o valor 0 é o default; NUNCA clicar) | `nh`, `t10`, `dh`, `sn`, `hs/tw` do board; `bo` = bundles |
