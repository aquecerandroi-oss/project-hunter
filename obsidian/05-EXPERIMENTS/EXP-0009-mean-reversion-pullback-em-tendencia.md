...
cripto na literatura, **mas não neste recorte**. A extrapolação de horizonte e de mercado é do
`quant-engineer` e está declarada.
...
- **Não é o experimento de política de saída.** "Sem invalidação" aqui é uma escolha de desenho sobre
  ...
  [[KB-0006-invalidacao-stop-por-atr-ou-saida-por-tempo]] exige (esse é o
  [[EXP-0007-momentum-invalidacao-bracos-INV]]).
- **Multiplicidade:** uma de quatro versões abertas no mesmo dia
  ([[KB-0010-overfitting-de-backtest-e-o-preco-de-cada-variante]]); T-036 do
