...
| [[EXP-0029-dispersao-btc-alts]] | `mean_reversion` `v20` (A, `dispersion=-0.10--0.03`) + `v21` (B, falseamento, `dispersion=0.00-0.10`) — **portão de dispersão**, parâmetros do pai `v10` byte a byte | 2026-09-12 | — (nenhuma coorte; a série `dispersion_24h_v1` ainda não tem uma linha) | **`nao-iniciado`** — pré-registro arquivado antes do backfill. Controle pré-declarado: o pai já medido (`v10`, `replay:c7d138eb…`, EXP-0025), que **não** será re-rodado, mais o braço B, mais a partição por `regime_hourly_v1`. Semente do bootstrap **20260913** (uma por experimento). Eixo primário `r_ex_funding` (cobertura de `R_net` da coorte do pai: 37,59 %); K4 lido no pai (0,93 %) |
