...
| [[EXP-0028-mean-reversion-5-min]] | `mean_reversion_m5` v1 — **módulo novo** (irmã de `mean_reversion_v1` que decide em 5 min; `atr_timeframe` 5m, `trend_timeframe` 15m — o degrau seguinte da grade, porque 4 × 5 min = 20 min não existe em `Timeframe` —, `horizon_s` 4800) | 2026-09-11 | — (ativada `research_only` 08:32:18Z; **nenhum replay rodado** — portão do `replay-worker` cego a shards) | **não iniciado** — P1 confirmada, P2 em parte e **P3 falsificada** (pedágio +0,0086 R contra a mãe, não +0,05…+0,12 R) sem uma decisão; regra de sucesso congelada: ex-funding 90 d > 0 com IC de blocos de dia acima de zero **E** PF > 1 em ≥ 2 de 3 janelas **E** leave-one-market-out nunca negativo **E** estresse não `frágil`; qualquer coisa menos é `descartar` com `--deprecate` na mesma tarefa. Braço de falseamento: a mãe **já medida** (`v1`, coorte `replay:fa005985…`, EXP-0025), que não será re-rodada |
