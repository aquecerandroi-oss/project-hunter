...
| `EXP-0027` | [[EXP-0027-amplitude\|a amplitude do universo como estado]] (`mean_reversion v18`/`v19` de `v10` — dois braços de **portão de amplitude** sobre a série `breadth_v2`, nenhum parâmetro muda) | T3.89 | **pré-registrada e avaliada em 2026-09-11/12**; distribuição medida somente-leitura (128 347 linhas usáveis, **cobertura 100 % nos 90 dias**, mediana 0,50, forma em **U**). **Descartada nos dois braços**: `v18` (A, "célula de dentro") Δ não pareado **−0,0184 R** contra o pai (sinal errado, IC [−0,1692;+0,1350], semente 20260912), 540/84; `v19` (B, falseamento) Δ **+0,0372 R** (abaixo do piso +0,05, IC [−0,1574;+0,2232]), 316/75. Condição 5 (Δ pareado por mercado-barra) **PASSA em 0,0000 R** nos dois — o portão é só um portão. **Cláusula de identidade dispara nos dois** (cortar o pai por `regime_hourly_v1` dá Δ igual ou maior: +0,0017 ≥ −0,0184 e +0,1456 ≥ +0,0372) — `breadth_v2` não supera o regime horário já descartado na EXP-0026. Estresse: `sem_vantagem_na_base` (A), frágil a custos (B). Ambos aposentados no mesmo dia (2026-09-12T01:15:24Z/01:16:02Z, 22:15/22:16 BRT) |
| `EXP-0028` | [[EXP-0028-mean-reversion-5-min\|a reversão à média decidida em 5 minutos]] (`mean_reversion_m5 v1` — **módulo novo**, `code_ref` próprio, o eixo de timeframe da EXP-0021 percorrido para **baixo**; três parâmetros mudam, os mesmos três da irmã de 1 h) | T3.84 | **pré-registrado em 2026-09-11, antes de qualquer replay**; semeado e ativado `research_only` em 2026-09-11T08:32:18Z, **replay bloqueado** (o portão do `replay-worker` lê `hb:strategy:shadow` e o grupo `strategy-worker.shadow`, que com `STRATEGY_SHARDS=4` ninguém escreve e ninguém consome: recusa `heartbeat_missing`/`consumer_lag:50000` com a faixa viva em `lag = 0`). **Avaliação parcial acrescentada em 2026-09-11**, sem uma única decisão: **P1 confirmada** (ATR%(5m) p50 medido **0,2714 %** em 90 d / 0,2984 % em 31 d contra 0,298 % previstos; controle de método reproduz os 0,5585 % de 15 m da T3.54 em 0,5582 %), **P2 com mecanismo confirmado e número falsificado** (o piso deixa passar **8,85 %** das barras de 5 m contra 33,04 % das de 15 m, mas o ATR% condicional é 0,7875 % e não os 0,60–0,75 % previstos) e **P3 falsificada** — o pedágio p50 é **0,2540 R** contra os 0,267–0,333 R previstos, e o Δ contra a mãe é **+0,0086 R** e não +0,05…+0,12 R. **O argumento de custo do pré-registro não sobreviveu à medição**: o veredito passa a depender só da expectativa bruta, que o replay mediria. C5 medido: 0,00 % dos stops abaixo do piso de 0,3 % e 1,11 % acima do teto de 3 % — a coincidência funciona, e custa **91,15 %** das barras (11 dos 16 mercados têm mediana de 5 m abaixo do próprio piso de risco). **Avaliação FINAL acrescentada em 2026-09-11, 07:42 BRT** (o portão foi consertado pela T3.87): coorte `replay:92c8d080-…`, 23 fatias, **414 720 barras, 0 erros**, **373 desfechos em 72 dias**. **Veredito `descartar`**, as quatro condições falham — ex-funding **−0,1940 R** com IC de blocos de dia **[−0,2889; −0,0943]** (todo abaixo de zero), PF 0,6971, PF > 1 em **1 de 3** janelas, leave-one-market-out **negativo nos 16 recortes**, estresse **`sem_vantagem_na_base`**. **P4 confirmada no centro da faixa** (previsto −0,25 a −0,09 R / PF 0,65–0,90) e **P5 confirmada** (373 em 160–810). E o achado que inverte o motivo: **o custo não foi a causa** — pela identidade `bruta − líquida`, o pedágio subiu **+0,0105 R** (0,2285 R contra 0,2180 R da mãe) enquanto a **vantagem bruta caiu de +0,1270 para +0,0346 R** (IC [−0,0625; +0,1387], indistinguível de zero): **89,8 % da piora é sinal, 10,2 % é custo**. Δ contra a mãe pareado por dia −0,1030 R **cruza zero** [−0,2670; +0,0686] — a coorte prova que esta versão é negativa, não que seja pior que a mãe. K1–K6 **não disparam** (K5 salta para 99,20 % de cobertura de `R_net`, contra 46,68 % da mãe, pelo horizonte de 80 min); ela morre pela regra de sucesso, mais exigente e escrita antes. **BTCUSDT e BNBUSDT não produziram nenhuma decisão em 90 d** (universo efetivo: 14 de 16). Versão **aposentada às 2026-09-11T10:54:08Z (07:54 BRT)**, no mesmo dia, `successor=none`. Portão de desenho `REVISE` (C5: o stop mediano de 1 ATR a 5 min cai em cima do piso de 0,3 % do `paper_v1`, e quem o salva é o `atr_pct_min = 0,006` herdado — logo o piso **define** a versão em vez de filtrá-la). Priors contrários pré-registrados com número: KB-0076 (pedágio p50 previsto 0,267–0,333 R contra os 0,2169 R da mãe), KB-0086 (460/460 células de custo negativas a 5 min no perp do BTC), EXP-0025 (a família de 15 m já é −0,0910 R em 90 d). Previsão registrada: **ex-funding −0,25 a −0,09 R, veredito `descartar`** |
| `EXP-0029` | [[EXP-0029-dispersao-btc-alts\|a discordância BTC × alts como estado]] (`mean_reversion` `v20`/`v21` de `v10` — dois braços de **portão de dispersão** sobre a série nova `dispersion_24h_v1`, nenhum parâmetro muda) | T3.90 | **pré-registrado em 2026-09-12, antes de o backfill rodar e antes de qualquer derivação**. Série nova (`market_dispersion`, `0020`): mediana do retorno de 24 h das alts **menos** o do BTC, por minuto fechado, universo sombra dos 16, referência `BTCUSDT` — a primeira série do repo com mercado de referência dentro do fold. Origem H-P18: 10/09 mediana −4,80 % contra BTC −1,50 % (dispersão **−0,033**) e 11/09 −3,85 % contra −1,30 % (**−0,0255**). Braço A = discordância `[−0,10; −0,03)`, braço B = falseamento `[0,00; +0,10)`; `[−0,03; 0,00)` (queda conjunta) e as caudas ficam **sem braço**, declarado. Régua da EXP-0027 congelada (Δ condicional × incondicional ≥ +0,05 R com IC de blocos de dia, n ≥ 100 e ≥ 30 dias, 2 de 3 janelas, LOMO nunca negativo, Δ pareado por (mercado, barra) dentro de ±0,02 R) **+ cláusula de identidade contra `regime_hourly_v1`** — e o pré-registro declara que **espera** a cláusula disparar, porque a dispersão é construída a partir do BTC. **Backfill rodado às 04:33 BRT de 12/09 (88 dias, 06-16 → 09-11, 126 720 linhas, zero inutilizáveis) e `‹backfill›` preenchidos às 04:40–04:41 BRT (T3.91).** **`inviavel-populacao` nos dois braços, nenhuma variante derivada:** a célula A cobre **1,48 %** das 8 352 barras de 15 min (124 barras em **9 dias**, 55 delas o próprio 10/09; zero barras em J2) contra os 10–30 % pré-registrados, e a célula B cobre **44,85 %** contra 20–40 %. A série é unimodal e estreita (mediana −0,0013, desvio 0,0132, p05 −0,0210): o corte −0,03 fica perto do percentil 1,5. Leitura descritiva no pai: A 7 decisões/2 dias; B 414/61 dias +0,0203 R; queda conjunta `[−0,03; 0)` 342/69 dias **−0,0721 R** (a pior célula); tercis não monótonos (+0,1502 / −0,0468 / +0,1016). Nada a aposentar (`v20`/`v21` não existem). H-P18 `testada` |
| `EXP-M1` | [[EXP-M1-comprar-cedo-na-curva\|comprar cedo na curva e vender em ROI alto]] (pump.fun; **série `EXP-M<n>`**, não a de quatro dígitos — conjunto de regras sobre a curva de bonding, não uma linha de `strategy_versions`) | Everton, 2026-09-12 ("comprar cedo na curva e vender em ROI alto") → T4.5, com os parâmetros do perfil `meme_paper_v0` de `docs/RISK_ENGINE_MEME.md` §3.1 (T4.4) | **pré-registrado em 2026-09-12, antes de existir uma linha de `meme_curve_snapshots`**. Entrada: idade 30–600 s **e** progresso 2–50 % **e** criador não vendedor líquido **e** participação ≤ 1 % do volume orgânico do último minuto; saída: 2× / trailing 30 % do pico / time stop 15 min / piso de perda 50 % / dump do criador / conclusão e migração da curva. Tamanho 0,05 SOL (`MEME_MAX_SOL_PER_TRADE`), que numa curva **é o risco da operação** (§5) — R = PnL ÷ custo, não distância de stop. Carteira **paper** (`hunter_indicators.meme`), sem chave e sem ordem real. Portão C1–C8 = **REVISE** (C3 amostra, C4 regime, C8 invalidação — falta instrumento, não desenho). Previsões congeladas: **P1** ≥ 95 % das portas recusam por insumo ausente (**7/7 já medido** nas criações reais da T4.1), **P2** marcação imediata **−3,4398 %** por construção da taxa (1,25 % da curva + 0,5 % do caminho), **P3** alvos entre 10 e 25 % contra os **33,3 %** de equilíbrio → **−0,30…0,00 R** e veredito previsto `descartar`, **P4** perda de fill ≥ 2 % dos tokens (mediana), **P5** < 100 desfechos em 30 dias. **Nenhuma corrida ainda — o coletor da T4.2 não escreveu nada** |
| `EXP-M2` | [[EXP-M2-a-linha-manda\|a linha manda]] (pump.fun; série `EXP-M<n>`) | Everton, 2026-09-12 ("você é obrigado a usar traçamento de linha") → T4.10, conjunto `trendline_v0/1` (migração `0026`) | **pré-registrado em 2026-09-12, antes de existir uma linha `meme_features_v3`**. Entrada: tudo o que a EXP-M1 exige, com idade ≥ 5 min, **e** dois últimos fundos locais ascendentes **e** mcap do minuto ≥ máxima da janela **anterior** de 15 min **e** preço 0–25 % acima da reta de suporte; saída: 2× / trailing 30 % / 900 s / **linha rompida** (2 fechos abaixo da reta projetada) / migração / dump. 0,05 SOL, tetos da EXP-M1. Linhas como feature (`hunter_indicators.meme.lines`, não-antecipação por `received_at`), nulos nomeados (`too_few_points`, `no_snapshot`, `flat`, `out_of_range`). Portão C1–C8 = **REVISE**. Previsões: **P1** < 5 % dos minutos com linha confirmada, **P2** mediana até `line_broken` < 4 min, **P3** −0,30…0,00 R (ponto −0,12) → `descartar`, **P4** Δ contra o controle dentro de ±0,05 R |
| `EXP-M3` | [[EXP-M3-sonda-de-hype\|sonda de hype]] (pump.fun; série `EXP-M<n>`) | Everton, 2026-09-12 ("os que não tiver [linha] você vai deixar já semi-comprado por causa do hype") → T4.10, conjunto `hype_probe_v0/1` (`0026`) | **pré-registrado em 2026-09-12**. Moeda com 30 s–5 min, `hype_score ≥ 0,6` (score documentado: compras 0,35, compradores 0,25, board 0,20, social 0,10, snipers baixos 0,10), `dev_share ≤ 10 %` ou desconhecido com motivo, snipers ≤ 2, criador não vendedor líquido, participação ≤ 1 % → **sonda** de 0,01 SOL (`leg = probe`; 3× / 40 % / 600 s); **escala** para a perna 2 de 0,04 SOL (`leg = scale`, `parent_bet_id`) só com a sonda aberta e a porta da EXP-M2 satisfeita, uma vez por sonda. Tetos: 2,0 SOL, 0,20/dia, 5 sondas abertas. Portão = **REVISE** (C1: o insumo é fabricável por wash trading). Previsões: **P1** ≥ 5 sondas/dia, **P2** ≤ 10 % escalam, **P3** sonda −0,40…−0,05 R (ponto −0,20) → `descartar`, **P4** perna 2 > sonda em ≥ 55 % dos pares mas ≤ 0 R, **P5** perda de fill ≥ 5 % |
| `EXP-M4` | [[EXP-M4-moonshot\|moonshot: mega ROI segurando através da migração]] (pump.fun; série `EXP-M<n>`) | Everton, 2026-09-12 ("pode colocar valores mais alto para sair num mega ROI, meme coin é diferente") → T4.11, conjuntos `moonshot_v0/1` (10×) e `moonshot_v0/2` (25×), migração `0029` | **pré-registrado em 2026-09-12**. A porta da EXP-M3 verbatim; 0,02 SOL; trailing 50 % **só depois de 3×**; espera 7 200 s; `exit_on_migration = false` — a posição sobrevive à migração e é **marcada pela fita da pool PumpSwap** (último trade − impacto por participação em 5 min, teto 1 % − taxa da faixa pelo mcap do momento − 0,5 % do caminho; venda no trade seguinte); saída `dead` (fita muda ≥ 15 min **e** marca ≤ 50 % da entrada, a zero sem trade em 3 min); sem piso de perda (declarado). Tetos 2,0 SOL, 0,20/dia, 8 abertas. Portão = **REVISE**. Régua: ≥ 100 apostas e 30 dias por braço, IC 95 % por blocos de dia, **leave-top-out obrigatório**. Previsões: **P1** ≥ 60 % morrem por `dead`/`time_stop`, **P2** ≤ 3 % chegam a 10× (≤ 1 % a 25×), **P3** braço 1 −0,90…−0,30 R (ponto −0,60) → `descartar` (H0: a cauda não paga as perdas somadas), **P4** leave-top-out negativo, **P5** ≤ 15 % chegam a ser marcadas pela pool |
| `EXP-M5` | [[EXP-M5-fluxo-e-holders\|fluxo e holders: a porta v2 e o relógio de 15 s]] (pump.fun; série `EXP-M<n>`) | Everton, 2026-09-12 ("aparecendo bastante proposta mas estamos perdendo todas; estamos muito lentos?") → estudo das 21 apostas (E1) → T4.16, conjuntos `flow_v2/1` (relógio de 15 s) e `hype_probe_v0/2` (minuto), migração `0030` | **pré-registrado em 2026-09-12, antes de existir uma linha `meme_features_15s`**. Porta: idade 30–300 s, `net_sol_flow > 0` (ou `mcap_delta_60s > 0`), ≥ 10 compradores distintos, `sells/buys ≤ 0,6`, holders subindo em duas leituras, progresso ≥ 5 % e subindo, snipers ≤ 2, dev ≤ 10 %, criador, participação ≤ 1 %; decidida **por fotografia de 15 s** e preenchida na seguinte; saída 3× / trailing 35 % após 1,5× / 1 800 s / dump / linha / piso 50 %; 0,05 SOL, 5 abertas. Braço 2: a sonda de hype + as três condições de fluxo. Placar só com desfechos `measured`. Portão = **REVISE**. Previsões: **P1** ≥ 60 % das recusas por cegueira da fita, **P2** decisão → fill p50 ≤ 15 s (era 27 s), **P3** idade na entrada ≤ 120 s, **P4** −0,40…+0,10 R (ponto −0,15) → `descartar` (H0), **P5** giro na entrada da sonda cai de 8/8 para ≤ 20 %, **P6** ≥ 30 % das recusas são pedigree |
| `EXP-M6` | [[EXP-M6-exclusoes-de-pedigree\|exclusões de pedigree: criador em série e clone de ticker]] (pump.fun; filtro transversal, não um conjunto) | estudo das 21 apostas de 2026-09-12 (E2: 6/21 criadores em série e 6/21 clones, todos perdedores) → T4.16, `exclusoes_de_pedigree v1` aplicado a todo conjunto | **pré-registrado em 2026-09-12**. `creator_prior_mints_1h ≥ 2` → `creator_serial`, `symbol_dup_24h ≥ 3` → `symbol_clone`, desconhecido recusa; contado em `meme_tokens` na hora da proposta, somado às recusas de toda porta e gravado em `reasons`. Portão = **PASS**. Previsões: **P1** ≥ 30 % das recusas nos 7 primeiros dias, **P2** desconhecidos ≤ 10 %, **P3** Δ dentro de ±0,05 R com a porta de fluxo ligada → `descartar` como filtro autônomo, manter como diagnóstico |
| `EXP-M7` | [[EXP-M7-organica-lenta\|orgânica lenta: entrar depois do pump dos bots e segurar pela migração]] (pump.fun; série `EXP-M<n>`) | 12/09: 35 apostas medidas a −9,4 R, `high_water_x ≤ 1,0` em 31 de 35, `creator_dump` em 22; M-L2 (< 5 % da curva custa −0,36 R); Everton 13/09 01:0x ("então aprimore essa parte") → T4.22, conjunto `organic_v0/1` (relógio de minuto), migração `0035` | **pré-registrado em 2026-09-13, antes de existir uma proposta**. Porta: idade 3–30 min, progresso 10–30 % e subindo, fluxo > 0, ≥ 5 compradores, vendas/compras ≤ 0,8, holders ≥ 20 subindo, top-10 ≤ 30 % (`max_top10_share`, novo), snipers ≤ 2, dev ≤ 10 %, criador, E2; saída 5× / trailing 40 % após 2× / 3 600 s / dump / piso 50 %, **segura pela migração** (`exit_on_migration = false`); 0,05 SOL, 3 abertas. Portão = **REVISE** (C2/C3/C4/C7). Previsões: **P1** ≥ 70 % das recusas por progresso/holders, **P2** ≤ 5 propostas/dia, **P3** ≥ 50 % das saídas por dump/piso, **P4** −0,30…+0,10 R (ponto −0,10) → `descartar` (H0), **P5** ≥ 1 aposta atravessa a migração em 7 dias |
| `EXP-M8` | [[EXP-M8-evento-que-pode-dar-bum\|evento que pode dar bum: a identidade social e o evento confirmado]] (pump.fun; série `EXP-M<n>`) | Everton, 16/09/2026 01:1x BRT ("as meme coins você vai mais por notícia — assim que uma moeda pode dar bum, que nem a do Trump") → T4.26, tabela `meme_events` + colunas sociais de `meme_tokens`, conjunto `event_v0/1`, migração `0041_meme_social` | **pré-registrado em 2026-09-16, antes de existir um evento casado**. Só propõe moeda ligada a um `meme_events` `confirmed` de `public_figure_launch`/`exchange_listing`/`brand_launch` (casamento por minuto, `handle_hint`/`symbol_hint` × `meme_tokens.twitter`/`symbol` nos 60 min seguintes); E2, dev ≤ 10 %, sniper ≤ 25 (largo de propósito), sem piso de fluxo; saída moonshot 10× / trailing 50 % após 3× / 7 200 s / dump / piso 50 %, segura a migração; 0,05 SOL, 5 abertas. Portão = **REVISE** (C3 amostra, C7 latência do casamento). Previsões: **P1** < 1 evento confirmed/dia nos primeiros 14 dias, **P2** maioria dos eventos nunca casa em 60 min, **P3** ≤ 3 propostas/14 dias, **P4** `inconclusivo` por amostra nos primeiros 30 dias, **P5** saída dominante `creator_dump`/`max_loss` antes do alvo |
| `EXP-M9` | [[EXP-M9-pedigree-e2b\|E2-b: a moeda que nasce cheia e a carteira que paga a subida sozinha]] (pump.fun; critério transversal medido, braço próprio) | [[11-KNOWLEDGE/KB-0103-clones-fundo-com-preco-forjado-assinatura-e-custo\|KB-0103]] (3 dias, 2 877 graduadas, rótulo de fita em 390: forjada = encheu ≤ 60 s **ou** maior comprador ≥ 35 %; a E2 v1 custa 51,3 % das orgânicas para pegar 34,9 % das forjadas) + [[11-KNOWLEDGE/KB-0105-e2b-replicacao-12-13-09-e-efeito-em-R\|KB-0105]] (replicação em 12–13/09: recall 92–100 % a custo 3,9–11,4 %; nas 103 apostas medidas marcaria 41, −14,2 R de −18,9 R, em dois dias perdedores) → T4.31, critério `pedigree_e2b/1`, conjunto `flow_v2/6`, migração `0044_meme_gate_e2b_arm` | **pré-registrado em 2026-09-16, antes de existir uma proposta do conjunto**. Recusa `e2b_born_full` (`completed_at − created_at ≤ 60 s`, só quando já observado) e `e2b_top_buyer_share` (≥ 0,35 do SOL comprado desde o mint, fita `meme_trades`), com guarda de ≥ 10 compradores; dado ausente recusa `e2b_top_buyer_unknown` e **é medido**. Base = a porta calibrada da mesa (16/09) + `pedigree_e2b`; `flow_v2/5` e `operator/5` seguem sem o critério. Portão = **REVISE** (C2/C3/C4/C7). Previsões: **P1** `e2b_top_buyer_unknown` 10–60 % das linhas (ponto 25 %), **P2** `e2b_born_full` ≤ 2 %, **P3** concentração marca 15–45 % das linhas com fita e ≥ 10 compradores, **P4** ΔR marcadas−não marcadas −0,60…+0,10 (ponto −0,20) com IC cruzando zero → `descartar` (H0), **P5** `flow_v2/6` propõe menos que `flow_v2/5`, mais por cobertura que por vício |
...
| [[EXP-0027-amplitude]] | `mean_reversion` v18 (A, `breadth=0.10-0.60@breadth_v2`) + v19 (B, falseamento, `breadth=0.60-1.00@breadth_v2`) — **portão de amplitude**, parâmetros do pai `v10` byte a byte | 2026-09-11 | `2026-09-12` (24 fatias, 270 336 barras, 0 erros; coortes `replay:f2c44f18…`/`replay:a94701c9…`; controle `v10` cortado em 06-14, `replay:c7d138eb…` não re-rodado) | **descartada nos dois braços** — condição 5 (Δ pareado por mercado-barra) **PASSA em 0,0000 R** nos dois, provando que o portão é só um portão; condição 1 **falha nos dois** (`v18` Δ **−0,0184 R**, sinal errado, IC [−0,1692;+0,1350]; `v19` Δ **+0,0372 R**, abaixo do piso, IC [−0,1574;+0,2232]); condição 3 falha nos dois (1 de 3 janelas); condição 4 falha nos dois (LOMO negativo em 16/16 e 1/16). **Cláusula de identidade dispara nos dois**: cortar o pai por `regime_hourly_v1` produz Δ igual ou maior (+0,0017 ≥ −0,0184; +0,1456 ≥ +0,0372) — `breadth_v2` não supera o regime horário do BTC. Estresse: `sem_vantagem_na_base` (`v18`), frágil a custos (`v19`). K3 dispara no pai e em `v18`. Ambos **aposentados no mesmo dia** (`--deprecate`, 01:15:24Z/01:16:02Z UTC = 22:15/22:16 BRT de 11/09) |
| [[EXP-0028-mean-reversion-5-min]] | `mean_reversion_m5` v1 — **módulo novo** (irmã de `mean_reversion_v1` que decide em 5 min; `atr_timeframe` 5m, `trend_timeframe` 15m — o degrau seguinte da grade, porque 4 × 5 min = 20 min não existe em `Timeframe` —, `horizon_s` 4800) | 2026-09-11 | `replay:92c8d080-6009-4a59-9868-31282b1bd493` (23 fatias, 414 720 barras, 373 desfechos, 72 dias) | **`descartar`** — ex-funding **−0,1940 R** IC95 [−0,2889; −0,0943], PF 0,6971, 1 de 3 janelas com PF > 1, LOMO negativo nos 16, estresse `sem_vantagem_na_base`; **aposentada no mesmo dia** (10:54:08Z). P4/P5 confirmadas; P3 falsificada duas vezes — a piora é **89,8 % sinal** (bruta +0,1270 → +0,0346 R) e só 10,2 % custo. Regra de sucesso congelada: ex-funding 90 d > 0 com IC de blocos de dia acima de zero **E** PF > 1 em ≥ 2 de 3 janelas **E** leave-one-market-out nunca negativo **E** estresse não `frágil`; qualquer coisa menos é `descartar` com `--deprecate` na mesma tarefa. Braço de falseamento: a mãe **já medida** (`v1`, coorte `replay:fa005985…`, EXP-0025), que não será re-rodada |
| [[EXP-0029-dispersao-btc-alts]] | `mean_reversion` `v20` (A, `dispersion=-0.10--0.03`) + `v21` (B, falseamento, `dispersion=0.00-0.10`) — **portão de dispersão**, parâmetros do pai `v10` byte a byte | 2026-09-12 | — (nenhuma coorte: a checagem de população §2 parou a EXP antes de derivar; série `dispersion_24h_v1` com 126 911 linhas usáveis, 06-16 → 09-12, lida às 04:40 BRT) | **`inviavel-populacao`** (`result: inconclusivo`) — braço A `[−0,10; −0,03)` = **1,48 %** das 8 352 barras de 15 min (124 barras, 9 dias, 0 em J2) contra 10–30 % previstos; braço B `[0; 0,10)` = **44,85 %** contra 20–40 %. `v20`/`v21` **não derivadas**, nada aposentado, zero replays. Previsão 1 acertou a forma (mediana −0,0013) e errou a cauda 10× (1,45 % abaixo de −0,03, não < 15 %); previsão 5 confirmada (0,00 % sem linha). Descritivo no pai (não move a régua): queda conjunta `[−0,03; 0)` é a pior célula (−0,0721 R, n = 342), B +0,0203 R (n = 414), tercis não monótonos. Controle pré-declarado (`v10`, `replay:c7d138eb…`) não re-rodado; semente 20260913 nunca usada |
| [[EXP-M1-comprar-cedo-na-curva]] | pump.fun — `comprar_cedo_na_curva v1` (porta) + `alvo_2x_trailing_30_tempo_15m v1` (saída), perfil `meme_paper_v0`; carteira paper `PaperCurveWallet`, R em **SOL** com risco inicial = **o valor gasto inteiro** (uma curva não tem livro onde pôr stop) | 2026-09-12 | — (nenhuma coorte; `meme_curve_snapshots` vazia nesta data) | **`nao-iniciado`** — pré-registro arquivado antes do coletor. Regra de sucesso congelada: expectância líquida em SOL > 0 com IC de blocos de dia (semente **20260912**) acima de zero **E** ≥ 100 desfechos e ≥ 30 dias **E** positiva em ≥ 2 de 3 janelas **E** leave-top-1 %-out ainda positiva **E** Δ contra o controle sorteado positivo **E** ≤ 20 % de censura; qualquer coisa menos é `descartar`. Custos nas duas pontas: **1,75 %** (1,25 % da curva, criador incluído, + 0,5 % do caminho PumpPortal local — §10.2: o papel nunca simula um caminho mais barato que o live) + priority fee + impacto da própria ordem; MEV, tip, rent e transação falhada **não** modelados — todo número é teto otimista. Os 23 checks de admissão da T4.4 ficam **fora** desta porta (bundled, top-10, frescura, impacto, slippage) |
| [[EXP-M2-a-linha-manda]] | pump.fun — `a_linha_manda v1` (porta: EXP-M1 + fundos ascendentes + rompimento da janela anterior + banda 0–25 % sobre o suporte) + `alvo_2x_trailing_30_tempo_15m_linha v1` (saída com `line_broken`), conjunto `trendline_v0/1`; R em SOL, risco = o gasto inteiro | 2026-09-12 | — (nenhuma coorte; `meme_features_v3` não existe na VPS nesta data) | **`nao-iniciado`** — pré-registro arquivado antes do deploy. Regra de sucesso: as seis da EXP-M1 **e** ≥ 100 minutos com linha traçável por dia em média; controle = a mesma porta sem os critérios de linha; identidade contra `mcap_slope_15m > 0`. Custos 1,75 % por ponta + impacto; MEV/tip/rent **não** modelados |
| [[EXP-M3-sonda-de-hype]] | pump.fun — `sonda_de_hype v1` (30 s–5 min, `hype_score ≥ 0,6`, dev ≤ 10 % ou desconhecido com motivo, snipers ≤ 2, criador, participação) + `alvo_3x_trailing_40_tempo_10m v1`; sonda 0,01 SOL, perna 2 de 0,04 SOL pela porta de `trendline_v0/1` com a sonda aberta; conjunto `hype_probe_v0/1` | 2026-09-12 | — (nenhuma coorte) | **`nao-iniciado`**. Regra de sucesso: as seis da EXP-M1 + Δ contra sondas sorteadas sem hype; para a perna 2, R maior que o da sonda no mesmo mint em ≥ 30 pares. K5: uma perna 2 sem sonda aberta invalida a coorte (o CHECK da `0026` torna o `parent_bet_id` nulo irrepresentável) |
| [[EXP-M4-moonshot]] | pump.fun — `sonda_de_hype v1` (a porta da EXP-M3) + `alvo_10x_trailing_50_apos_3x_tempo_2h v1` (`moonshot_v0/1`) e `alvo_25x_trailing_50_apos_3x_tempo_2h v1` (`moonshot_v0/2`); 0,02 SOL; segura através da migração, marcada pela fita da pool (`pool_mark_sol v1`: impacto por participação, faixa da PumpSwap pelo mcap); `dead` a zero; sem piso | 2026-09-12 | — (nenhuma coorte; nenhuma linha `pump_amm` em `meme_trades` nesta data) | **`nao-iniciado`**. Regra de sucesso: as seis da EXP-M1 **e** leave-top-out positivo **e** Δ contra a EXP-M3 sem identidade, por braço. K1: > 30 % das migradas sem trade da pool em 15 min = instrumento cego; K5: uma aposta de `moonshot_v0` fechada como `migrated` invalida a coorte |
| [[EXP-M5-fluxo-e-holders]] | pump.fun — `fluxo_e_holders v1` + `alvo_3x_trailing_35_apos_1_5x_tempo_30m v1` (`flow_v2/1`, relógio de 15 s: `meme_features_15s`, fill na fotografia seguinte) e `sonda_de_hype v2` (`hype_probe_v0/2`, a sonda + fluxo); R em SOL, risco = o gasto inteiro; placar só com `measured` | 2026-09-12 | — (nenhuma coorte; `meme_features_15s` não existe na VPS nesta data) | **`nao-iniciado`**. Regra de sucesso: as seis da EXP-M1 **e** Δ contra a porta sem fluxo/holders (braço 1) ou contra `hype_probe_v0/1` (braço 2) **e** leave-top-out, ≥ 100 apostas e 30 dias por braço. K1: > 80 % de recusas por cegueira da fita = consertar a cobertura, não relaxar a porta |
| [[EXP-M6-exclusoes-de-pedigree]] | pump.fun — `exclusoes_de_pedigree v1` (filtro transversal de toda porta: `creator_serial`, `symbol_clone`, desconhecido recusa) | 2026-09-12 | — (nenhuma proposta prospectiva com o filtro nesta data; retrospectiva: 11 das 20 perdedoras de 12/09 cairiam) | **`nao-iniciado`**. Regra de sucesso: Δ ≥ +0,10 R contra "sem pedigree" cortando ≤ 40 % das entradas, ≥ 100 propostas e 30 dias; menos que isso, `descartar` como filtro autônomo e manter como diagnóstico |
| [[EXP-M7-organica-lenta]] | pump.fun — `organica_lenta v1` + `alvo_5x_trailing_40_apos_2x_tempo_60m_segura_migracao v1` (`organic_v0/1`, relógio de minuto, `exit_on_migration = false`); R em SOL, risco = o gasto inteiro; placar só com `measured` | 2026-09-13 | — (nenhuma coorte; conjunto semeado pela `0035` nesta data) | **`nao-iniciado`**. Regra de sucesso: R médio ≥ +0,20 com leave-top-out ≥ 0, ≥ 100 apostas e 30 dias, IC 95 % por blocos de dia; abaixo disso `descartar` |
| [[EXP-M8-evento-que-pode-dar-bum]] | pump.fun — `evento_pode_dar_bum v1` + `moonshot_10x_trailing_50_apos_3x_2h_segura_migracao v1` (`event_v0/1`, relógio de minuto, `exit_on_migration = false`); R em SOL, risco = o gasto inteiro; placar só com `measured` | 2026-09-16 | — (nenhuma coorte; `meme_events` semeada vazia pela `0041_meme_social` nesta data) | **`nao-iniciado`**. Regra de sucesso: R médio ≥ +0,20 com leave-top-out ≥ 0, ≥ 100 apostas e 30 dias, IC 95 % por blocos de dia; abaixo disso `descartar`. Pré-condição declarada: o adendo do plantão (camada 3, brief T4.26) precisa produzir eventos `confirmed` suficientes — sem isso a régua nunca fecha |
| [[EXP-M9-pedigree-e2b]] | pump.fun — critério `pedigree_e2b v1` (`e2b_top_buyer_share_max` 0,35, `e2b_min_buyers` 10, `e2b_born_full_s` 60) sobre o gate `fluxo_e_holders v3` (`flow_v2/6`, relógio de 15 s, `research_only`); R em SOL, risco = o gasto inteiro; placar só com `measured` | 2026-09-16 | — (nenhuma coorte; conjunto semeado pela `0044_meme_gate_e2b_arm` nesta data) | **`nao-iniciado`**. Regra de sucesso: ΔR (marcadas − não marcadas) ≤ −0,30 com IC 95 % por blocos de dia inteiro abaixo de zero, ≥ 100 apostas medidas e 30 dias; abaixo disso `descartar`. Confundimento declarado: `flow_v2/6` carrega também a calibração da mesa de 16/09 — o contraste limpo é dentro do próprio conjunto (recusas `e2b_*` contadas por nome) |
...
Quatro braços nasceram dos estudos quant de 16/09 **com pré-registro escrito antes de existir qualquer proposta** —
...
**Nenhum dos quatro está implantado**; os números abaixo são in-sample, das KBs citadas.
...
- **EXP-M22** — absorção de venda (entrar só depois que uma venda ≥ 5 % da reserva real é recomposta em 30 s e sustentada 10 s; controle entra logo após a mesma venda): pré-registrado 20/09 (parecer da Astra, item 4); braços `absorb_v0/1` (tratamento) e `absorb_v0/2` (controle) semeados em papel na `0058` (T4.79); leitura em 3 dias / ≥ 60 apostas por braço; ver [[EXP-M22-absorcao-de-venda]] · [[KB-0143-o-que-antecede-o-dump]]
- **EXP-M24** — recuo pequeno como melhora de preço (**H-017**, coorte nova; a H-016 refutou no R77, [[KB-0157-esperar-o-recuo-nao-paga]]): braço de papel `recuo_v1/1` semeado na `0063` (T4.91) com a porta efetiva de `operator/5`, saída da mesa e a célula **3 % / 60 s**; emparelhado com a sombra de `operator/5`, `no_pullback` = 0, também contra "não comprar nada"; julgamento com ≥ 150 decisões resolvidas; a leitura de 10 entradas é só verificação do mecanismo; ver [[EXP-M24-entrada-no-recuo]]
