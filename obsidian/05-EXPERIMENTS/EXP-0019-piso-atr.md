...
> **Arquivado pela Sexta-feira em 2026-09-08 (T3.47b)** a partir do rascunho do `quant-engineer`
> (`.claude/state/exp-drafts/EXP-0019-piso-atr.md`, T3.47b). "Hipótese", "Braços", "Portão" e
> "Protocolo" vêm do rascunho **sem alteração de conteúdo** e são as seções **congeladas**;
> avaliações são **acrescentadas** abaixo, datadas.
>
> **Correção de vocabulário controlado, declarada:** o rascunho do quant-engineer escreveu
> `result: negativo` no frontmatter — descrição fiel do sinal dos quatro contrastes
> pré-registrados, todos negativos, e do estresse (`frágil a custos` nos dois braços). Mas o
> próprio `_TEMPLATE-EXP.md` e a regra de maturidade do `docs/plans/SHADOW-LAB.md` §9 exigem
> `inconclusivo` **obrigatoriamente** abaixo de 100 outcomes avaliáveis **e** 30 dias distintos —
> aqui são 33 e 37 avaliáveis (< 100) em 11 dias (< 30), e nenhum dos quatro IC exclui zero. O
> vocabulário do vault (`result` em `infra/scripts/obsidian_lint_rules.py`) também não aceita
> `negativo` como valor. Corrigido para `inconclusivo` **só no campo controlado**; o veredito
> qualitativo do corpo do EXP ("Formal: `negativo` nos dois braços", abaixo) fica **verbatim**,
> porque descreve o sinal medido, não o rótulo de maturidade — e a diferença entre os dois é,
> ela mesma, o achado que o [[KB-0010-overfitting-de-backtest-e-o-preco-de-cada-variante]] cobra.
>
> **A frase honesta que este EXP deixa, verbatim:** o piso de ATR% não escondia decisões boas —
> escondia **dias ruins**. Os quatro dias que só existem com o piso baixo são os quatro negativos;
> baixá-lo compra 18 operações a **−3,94 USDT cada**. Somado ao [[EXP-0013-momentum-alvo-3-atr]]
> (alvo) e ao [[EXP-0018-stop-largo]] (stop), fecham-se três eixos de geometria medidos, e nenhum
> fabricou vantagem: **falta vantagem na entrada, e nenhuma geometria a inventa.** Ligada a partir
> de [[Experiments Index]], [[EXP-0017-sweep-reclaim]] e [[KB-0076-por-que-perdemos-2026-09-08]].
...
> do Lab, [[KB-0076-por-que-perdemos-2026-09-08]]).
...
[[EXP-0018-stop-largo]]. Nenhum outro parâmetro muda: teto de ATR%, horizonte, atraso máximo de entrada, custos
...
medidas pelo EXP-0014 (ainda não arquivado no vault) e por [[EXP-0018-stop-largo]]:
...
| C2 | Risco de sobreajuste | **PASS** | Nenhum limiar novo foi garimpado: 0,006 é o valor **original** do código congelado, e 1,5 é a escala já pré-registrada no [[EXP-0018-stop-largo]] B1. Zero graus de liberdade escolhidos depois de ver o dado. A alternativa 0,004 **não** foi testada, e está pré-declarada abaixo em vez de tentada |
...
| C8 | Qualidade da invalidação | **PASS por ausência** | A `mean_reversion_v1` **não tem invalidação estrutural** (0 `invalidated` em todas as oito coortes da linhagem), então o eixo do piso não interage com o vício que o [[EXP-0018-stop-largo]] C8 documentou na `momentum`. O livro tem três modos de morrer: alvo, stop e horizonte. O piso baixo desloca o mix de 80 % horizonte (`v6`) para 54,5 % horizonte / 27,3 % stop / 18,2 % alvo |
...
## Critérios de morte (K1–K6, régua do [[EXP-0017-sweep-reclaim]]) — escritos antes da leitura
...
> **Rótulo obrigatório: REPLAY.** Mesma janela que gerou a hipótese ([[KB-0010-overfitting-de-backtest-e-o-preco-de-cada-variante]]). Não é evidência
...
os dois por fragilidade a custo. Nenhum IC exclui zero, então a régua prospectiva do `docs/plans/SHADOW-LAB.md`
...
**O que isto faz com o [[EXP-0018-stop-largo]] B1 (`mean_reversion v6`):** a recomendação daquele EXP era
...
medição não existe desde a [[KB-0076-por-que-perdemos-2026-09-08]] e vale mais do que qualquer variante nova.
...
afrouxá-lo é comprar 18 operações a −3,94 USDT cada. Somado ao [[EXP-0013-momentum-alvo-3-atr]] (alvo) e ao
[[EXP-0018-stop-largo]] (stop), fecham-se **três eixos de geometria medidos, e nenhum fabricou vantagem**: os
