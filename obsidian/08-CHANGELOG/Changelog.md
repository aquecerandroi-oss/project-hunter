...
*(**Acréscimo do arquivamento da Sexta-feira em 2026-09-08 (mais tarde na noite): os 25 commits
seguintes**, `7b0edeb..402c56b`, mais novo primeiro — o bloco acima parou em `7b0edeb` (a própria
filial de Obsidian da T3.41). Agrupados por tema em vez de por hora, porque o dia misturou seis
frentes ao mesmo tempo: o **Lab** ganhou visualização (linhas de tendência e stop sobre velas reais
em cada operação, 681 gráficos exportados), o **tempo real** do navegador teve o bug do topbar
resolvido (WebSocket fechava a cada 25–50 s por código 4408 silencioso) e o painel de Execução
corrigido, **segurança/banco** fechou o HIGH-1 (role `hunter_runtime` sem superusuário, DSN de dono
fora dos workers) e criou a migração de `exchange_status = planned`, **estratégias** mediu três eixos
de geometria (alvo, stop, piso de ATR%) sem achar vantagem nova e abriu duas famílias
(`trendline_breakout`, `sweep_reclaim`), **Radar/regime** entregou o produtor de regime horário e
concluiu que o Radar (1,8 dia de idade) não prevê nada ainda, e a proposta de limites de risco foi
escrita para o Everton decidir.)*

**Lab**

- `402c56b` — T3.49: no detalhe de cada operação do Lab, 'Ver linha de tendência' desenha sobre velas reais a linha usada na decisão (sólida até a decisão, pontilhada até a saída), o pivô do stop, entrada/stop/alvo limitados ao trecho da operação e a saída com motivo; painel 'Geometria' com os campos gravados; sem ponto fabricado (nota honesta quando falta cobertura); 15m nativo não existe em lugar nenhum → agregação de 1m real com a regra de balde completo (count = 15, mesma do T3.50 — ressalva da revisão fechada); 38 testes; prova visual local bloqueada (Chromium não resolve loopback nesta máquina desde ~21:10) — conferir na VPS
- `6a8679b` — T3.50: toda operação concluída do Lab ganha o seu gráfico traçado — 681 PNG (44 MB) de 7 versões em obsidian/attachments/operacoes, uma nota por versão em 03-TRADING/Operacoes-tracadas + README (só trendline_breakout lê linhas; nas outras as linhas são contexto do mesmo tl_scan congelado, cortado na barra da decisão, sem antecipação); render_operations.py export/render/note (idempotente, ≤ 200 por rodada, matplotlib só via uv run --with); prova: 47/47 line_id e preço da linha da trendline_breakout reencontrados em Decimal exato; PIPELINE §9b + marcação do custo obsoleto do regime (T3.43c); briefs T3.49/T3.44e
- `95bc2f0` — T3.51 + T3.44d/e/f: abas e paginador do Lab viram links reais (URL = fonte do filtro; router.push de botão nunca commitava nesta página) e a atualização automática deixa de atropelar navegações (startTransition + guardas: visibilidade, refresh pendente, navegação recente — primeiro tick após montar nunca é descartado, achado da revisão); tempo real do topbar: pong centralizado no cliente (o canal do Radar era derrubado a cada 25–50 s por 4408), diagnóstico lastClose/since, carência de 8 s ancorada no último 'open' (queda de minutos nunca lê 'ao vivo'), ping de cliente a cada 5 min com aba visível (o servidor fecha por ociosidade 4409 aos 15 min — 10/h no IP do Everton), tooltip com motivo e hora de Brasília; revisões: T3.51 ressalva fechada, T3.44 bundle APPROVE
- `3bd8f3d` — T3.18d: um veredito, uma função — compute_verdict delega a verdict_from_values do pacote puro (o motivo do PF nulo tem um único ponto de tradução no_losses ↔ sem_perdas); matriz de 8 casos-limite; teste do placar excluindo linha terminal madura com R nulo de evaluable e de maturity.days; 26 unit + 16 + 3 integração verdes
- `c6cb696` — state: briefs T3.18d (veredito em uma função só + teste do placar com R nulo) e T3.45 (contrato sweep + reclaim com pré-checagem)

**Tempo real**

- `8770ff4` — T3.44: tempo real do navegador — ws.ts não manda mais token nulo na corrida com o getToken do Clerk (loga, fecha e reconecta pelo backoff) e só marca 'open' ao receber {type: authenticated}; System: painel Execução paper lia a heartbeat genérica role=execution (ordenada antes de 'paper') — agora filtra instance=paper; 966 testes verdes. Achados: 'UNAVAILABLE' vem da bybit cadastrada active sem worker (honesto, decidir status); endpoint WS fecha 4401 sem log (backend)
- `2e39774` — T3.44b: o gateway WS loga todo fechamento (ws_closed code/reason/client/authenticated/duration_ms) e conta hunter_ws_closed_total{code} — o 4401 silencioso que escondeu o bug do topbar por dias deixa de existir; admission.py extraído; 36 unit + 4 integração verdes; Bybit planned bloqueada pelo enum exchange_status (brief T3.44c para database-architect); brief T3.46 (o Radar prevê?)
- `f6d222f` — state: brief T3.44b (log de todo fechamento WS; bybit planned fora do agregado)

**Segurança/banco**

- `a9bacc6` — T3.15d/e/f/g + T3.44c + T3.45b: o runtime deixa de carregar a DSN de dono (só migrate/ops; subcomando compose.sh ops nunca constrói; preflight recusa DATABASE_URL_MIGRATIONS no .env; cron de partições via ops; lint dos runbooks), migração 0015 cria hunter_runtime (LOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT, membro só de hunter_app/hunter_worker, sem senha no repo; runbook (a)–(e) com \password; resíduo do login único documentado — dois logins = T3.15h; lint contra engine.begin cru), migração 0016 exchange_status 'planned' (Bybit planejada fora das linhas/contagens/Redis; seed --only exchanges; seed_risk_reference.py extraído), sweep_reclaim_v1 (stop estrutural, porta de custo por risco% 0,006, teto 0,3333 R, 46 testes, digests vivos intactos). Revisões: security T3.15e/T3.15f aprovadas com ressalvas fechadas; T3.45b aprovada
- `0cb6fc9` — state: briefs T3.15e (cron de partições via ops, cap_drop, preflight, default vazio da DSN de dono) e T3.15f (login role hunter_runtime sem superusuário — fechamento real do HIGH-1)

**Estratégias**

- `803f648` — T3.47c: --deprecate e --supersede acrescentam '[deprecated <iso>] <texto>' ao changelog em vez de sobrescrever — a linhagem derived_from/overrides/params_hash da variante sobrevive à aposentadoria (append_deprecation_note, 6 unit + test_deprecate 14 + test_supersede 12 verdes); ACTIVATION §7b
- `7e9d59c` — T3.47b: piso de ATR% — baixar de 0,008 para 0,006 dobra as decisões (15 → 33) e destrói a expectância (+0,286 → +0,086 R): a faixa nova rende −0,08 R nas duas larguras de stop; o piso era a margem de segurança de custo (custos ×2 vira negativo), não escondia decisões boas, escondia dias ruins; a 'v9' do brief é a própria v1 (recusa auditada); roster 16 → 14 (momentum v7, mean_reversion v4/v5 aposentadas pelo script, acompanhamentos abertos seguem); mean_reversion v8 (piso 0,006 + stop 1,5) é superconjunto exato da v6 — candidata prospectiva por população, v6 por mérito; defeito achado: --deprecate sobrescreve o changelog e apagaria a linhagem (brief T3.47c)
- `1c72134` — T3.47: stop largo — pedágio cai pela metade (0,255 → 0,131 R na momentum v6) mas R encolhe junto: sobra +0,016 R/op (12 % da economia), IC contém zero; com risco fixo em 0,25 % dobrar o stop = metade da posição (menos risco, não mais); mean_reversion v6 (piso 0,008 + stop 1,5 ATR) guarda 95 % da economia, PF 2,28, 15 decisões (K1); v7 momentum e v4/v5 mean_reversion a descartar; variantes põem 20–44 % das decisões acima do teto de stop do paper_v1 (C5); próxima alavanca = piso de ATR% (proposta em EXP-0018 §15); blocos_pareado.py (IC pareado por bloco de dia)
- `9ff2d47` — T3.34c: trendline_breakout_v1 no catálogo da VPS (seed --only strategies, ativada research_only 19:29 BRT, digest 7b83a1ff…), replay 31 d × 4 mercados: 47 decisões, bruto +0,10 R, líquido −0,04 R, PF 0,92, pedágio 0,14 R; 89 % das decisões são repique (K6) — hipótese reenunciada, 'manter em pesquisa'; invalidação estrutural repete KB-0006 (Δ +0,09 R sem ela, IC contém zero); ressalvas da revisão de c9691d0 fechadas (36 parâmetros, KB-0006/EXP-0007 em C8); cada decisão grava line_id/inclinação/índices/preço/pivô — base da T3.49; aceite do toll_cap_r derivado (T3.45b)
- `c9691d0` — T3.34b: trendline_breakout_v1 — primitivos de linhas de tendência portados para módulos irmãos planos (tl_pivots/tl_lines/tl_events/tl_scan com retire_after_break) com paridade Decimal exata contra hunter_indicators.patterns (33 testes), tl_setup (gatilho + envelope: rompimento de resistência descendente válida com RVOL ou repique em suporte ascendente; stop abaixo do último pivô com piso 2 ATR e teto 3 ATR; alvo = largura do canal ou 2 R; invalidação = fechar de volta abaixo da linha), 37 parâmetros congelados, max_violations=1 (a barra que rompe é uma violação), 32 testes próprios + 5 de não-antecipação; digests das versões vivas intactos; portão C1–C8 = REVISE (autoavaliação; K1 é risco real)
- `b72c423` — T3.45: sweep + reclaim — contrato EXP-0017 (stop estrutural, porta de custo por risco% = 0,006 em vez de piso de ATR, teto 0,3333 R, sem invalidação), pré-checagem na VPS passou por margem estreita (57 eventos em 31 d × 4 mercados, 17 dias distintos, 30 % no maior mercado, cobertura 99,5 %), portão C1–C8 = 70,5 REVISE (C8 fail por desenho), brief de implementação T3.45b; achado: atr_pct_min 0,006 recusa 67–89 % das barras desses mercados (breakout/mean_reversion carregam esse piso)
- `259d6f6` — T3.42: mean_reversion v2 (piso 0,008) e v3 (piso 0,010) derivadas, ativadas (18:24/18:34 BRT) e replayadas — subconjuntos exatos do pai (Δ pareado 0): v2 17 decisões +0,30 R líq PF 1,75, v3 11 decisões +0,51 R PF 2,69; custos ×2 vira positivo nas duas; MAS IC por dia contém zero, 11 decisões da v3 cabem em 4 dias (filtro de regime disfarçado), v3 corta 70,3 % (régua EXP-0006 dispara) → inconclusivo, manter em pesquisa (v3 com ressalva forte); brief T3.43b (tile de regime horário não é stale)
- `2df9f67` — state: briefs da leva da noite — T3.42 (irmãs da mean_reversion por teto de pedágio), T3.34b impl (trendline_breakout_v1 com primitivos portados), T3.43 (produtor de regime), T3.44 (stream do topbar e painel Execução na VPS), T3.15d (DSN de dono fora dos workers)

**Radar/regime**

- `28b28fa` — T3.43b + T3.46c: tile 'Regime atual' com rótulo em português, score/confiança, hora de Brasília e os cinco componentes (stale só se a hora tem > 2 h ou o heartbeat regime_last_ts parou; regime_v0 intocado; 26 unit + 4 integração); página do Radar/Opportunities diz a verdade sobre o próprio estado — GET /api/v1/radar/coverage (25/217 mercados, ponteiro do bootstrap, baselines prontas, 12 detectores produzindo/desarmado/silencioso, score máximo 38 < degrau 40, condições de reabertura com progresso real); api.d.ts regenerado (carrega também exchanges_planned da T3.44c já em main); revisões aprovadas com ressalvas (nota de custo dos agregados sem índice registrada; teste de fronteira do heartbeat deixou de ser flaky)
- `dfe6116` — T3.46 + T3.48: o Radar não prevê (1,8 dia, 25/217 mercados, 8 % das baselines, score máximo 38 < degrau 40, IC por blocos todos cruzando zero) — rebaixado a painel com condição de reabertura (KB-0078 arquivada); proposta de limites de risco para Everton: risk_per_trade 0,25 % é inerte (participação 1 % do volume vence em 164/176 mercados, risco real 0,0067 %/op), recomendação manter e condição de subida escrita; briefs T3.34c/T3.44d/T3.46c/T3.47/T3.48/T3.50
- `14c4b54` — T3.43c: regime horário se repara de verdade — janela de reparo de 72 h com digest antes da transação (só reescreve o que mudou; xmin das horas intocadas não muda), escrita em leva por dia com repetição hora a hora (backfill de 31 d: 186,5 s → 11–12 s), --repair-days, status_details.pop('regime_hourly') no finally, teste de dois produtores no mesmo corte; revisão: aprovado com ressalva (custo de leitura de 97 h a remedir, marcado obsoleto no PIPELINE §4b item 9 — segue no próximo commit de docs); brief T3.51 (abas do Lab não respondem)
- `f7aed39` — T3.43: produtor horário de regime — hunter_indicators.regime (hourly_model/hourly/hourly_score/hourly_snapshot: tendência do BTC por SMA50/200 e inclinação, vol realizada 24 h ranqueada em 720 h, amplitude do universo, funding médio, drawdown 30 d; score 0–100 com pesos redistribuídos e confiança; limiares versionados; sem antecipação provada), job horário no scanner-worker (regime_repo/writer/job/hourly: dobra em SQL só de minutos finais, upsert por (exchange, ts) com digest, backfill por ausência, heartbeat regime_last_ts, métricas), SQL de corte por regime, PIPELINE §4b; 30 + 6 testes verdes; ensaio a seco: 745 horas do BTC em 1,7 s
- `e6ba001` — state: brief T3.43c (janela de reparo do regime, pop no finally, teste de dois produtores, custo medido)
- `69ae37c` — hotfix T3.44c: market_status.py (extraído de system_status.py na T3.44c) ficou fora do commit — a API na VPS entrou em loop de reinício por ModuleNotFoundError; arquivo adicionado

**Obsidian**

- `7b0edeb` — T3.41: Obsidian fim do dia — EXP-0010 avaliada, EXP-0012 descartada por construção, EXP-0013 mantida, breakout v2 arquivada e v1/v2 aposentadas, estresse da mean_reversion, nota da identidade de custo, backlog B7/B8, diário com o placar das sete versões, Changelog com os 71 commits do dia, Open Bugs
...
- `98659338` diario(meme): 19/09 tarde/noite — 24 ops, 9 alvos, −0,040 SOL; operator/6 1.º dia 2 compras +0,004; recompra do mesmo mint 25 s após recuo (candidata a pausa por mint); 429/unsupported_quote no pump.fun
- `6a2e91b5` diario(meme): 19/09 22:3x — poda do Lab proposta (todos os braços negativos; flow_v2/1 −0,21/24 h, base da operator/6 evaporou), R64 despachado, aprendizado das 24 reais
- `6c9c5df1` fix(meme-worker): T4.70b — subscribe_at_create engole ConnectionError/TimeoutError/OSError por mint (derrubou o radar às 18:05Z de 19/09), contador event_gate_subscribe_at_create_failed_total + log limitado a 1/30 s; 7 testes
- `4464c8dc` feat(meme-executor): T4.74-4 — caminho do dinheiro da spot/1: spot_leg (quote → swap com prioridade capada → verify → simulate com accounts → invariante → chave relida → assina UMA vez → assinatura gravada antes do envio → fill pelo meta da própria tx, nunca confirmed com 0), entradas pelo sinal (evaluate_spot_entry, 1 por tique, refutação/cooldown), regras de saída puras; JupiterClient.swap(max_priority_fee_lamports); 75 testes; 2 must-fix da Astra fechados
- `622bfe0e` research(meme): R64 — anatomia das 24 reais de 19/09: 75 % da perda é aluguel de ATA (flag de fechar ATA desligada; 34 ATAs = 0,051 SOL parados); regra atual é a 1.ª de 52 braços (drenagens de ≥ 70 % num bloco punem recuos largos); recompra do mesmo mint em papel 0/19 → [[KB-0146-trailing-apertado-e-rent-de-ata|KB-0146]] · [[EXP-M21-pausa-por-mint-apos-trailing|EXP-M21]]
- `7eecb90d` feat(ops): T4.77 — script auditado meme_close_atas.py: fecha ATAs vazias do Token clássico (dry-run padrão; --apply com chave ACTIVE relida por lote, verificador puro só ComputeBudget+CloseAccount para a carteira, simulação com invariante, assinatura commitada antes do envio, recuperado lido do meta da tx); Token-2022 (as 35 do pump.fun = 0,0515 SOL) só listadas — decisão pendente; 74 testes; ACTIVATION §9k
- `e17a10af` decision(meme): 19/09 23:2x — T4.77b autorizada (fechar ATAs Token-2022 vazias com guardas)
- `71b595e8` feat(meme-executor): T4.74-5 — saídas da spot/1 (marca por cotação do lote, emergency>sell_requested>stop>target>time, backoff/pânico, 6 falhas duras ⇒ bloqueada), reconcile por assinatura (compra tardia abre posição, expira só após 2.ª olhada acima do last_valid_block_height, reparos de marcador/órfãs), settle ao lamport, heartbeat spot1, fiação em config/context/main/heartbeat/admissão (posições e compras spot em voo contam no freio global); refutação pelo pior prefixo; 630 testes (+67) + 1 integração; 8 achados da Astra fechados
- `b3d03fc2` review(spot): T4.74 — deploy inerte aprovado; ligar aprovado com ressalvas (A1 gestão com flag off, A2 par rejeitado para sempre); diário
- `90a7f997` feat(ops): T4.77b — meme_close_atas.py --token-2022: fecha ATAs Token-2022 vazias só com type=account, extensões ⊆ immutableOwner, space exato (165/170), ATA derivada no programa certo, lotes homogêneos, verificador exige programa = plano por conta; modo-prova de 1 conta (--i-know-2022 libera o resto); 131 testes; ACTIVATION §9k-2022; 2 rodadas da Astra
- `fab5d30e` fix(spot): T4.74-7 — saídas da spot/1 rodam sempre com live+signer (a flag só governa entradas); erro não-retryable da Jupiter na venda vira recusa dura (conta nas 6 ⇒ blocked_exits); 30 transitórias ⇒ stuck_exits no heartbeat; posição fechada no meio ⇒ nada assinado; --close-manual auditado (meta da tx, carteira ancorada à compra, FOR UPDATE, rollback no Refused); 644 + 83 testes; 4 must-fix da Astra fechados
- `85036408` diario(meme): 19/09 fechamento — 26 ops, 9 alvos, −0,060 SOL; spot/1 pronta para deploy inerte; pacote 10 no GitHub; pendências do Everton → [[Diario/2026-09-19]]

## 2026-09-20

- `aab81473` feat(meme): T4.78 — pausa por mint após perda na mesa real: check 28 mint_cooldown_after_loss (último da fila, mesa e lançamento, vendas nunca), MEME_MINT_COOLDOWN_AFTER_LOSS_S=300 (0 desliga), leitura por mint, robô espera o restante anunciado, contador no heartbeat; 1149 testes; 2 must-fix da Astra; EXP-M21 'aplicada no real'; RISK_ENGINE_MEME §4 → [[EXP-M21-pausa-por-mint-apos-trailing|EXP-M21]]
- `7dea37df` decision(meme): 20/09 00:3x — T4.78 pausa por mint 300 s após perda na mesa real, por exigência do Everton; contraponto NARKY registrado; medição em sombra
- `2f066fe8` diario(meme): 20/09 madrugada — Astra: nada muda na mesa real hoje; EXP-M22 absorção de venda pré-registrada (T4.79); .env do aluguel ainda não pegou → [[EXP-M22-absorcao-de-venda|EXP-M22]] · [[Diario/2026-09-20]]

## 2026-09-22

*(sem commits em 2026-09-21 — a mesa ficou parada por rate limit do RPC da Solana; nenhum diário próprio dessa data.)*

- `334a4a52` diario(meme): 22/09 — mesa parada há 30 h (RPC da Solana em rate limit desde 21/09 12:08 BRT); balanço de 6 dias −0,347 SOL; aluguel de ATA nunca desligado; R65 despachado
- `746fe1be` diario(meme): 22/09 — R65 no diário: 72 % do prejuízo é custo, rent de ATA é 33 %; sem ele o acerto de equilíbrio cai para 19 % (a mesa fez 26 %); nenhuma variável de gate separa; buys_1m vai para sombra
- `2db09961` decision(meme): 22/09 — Everton autoriza ligar o fecho de ATA e a mesa spot/1 (1 posição); ordem travada pelo RPC em rate limit
- `c56f4017` diario(meme): 22/09 — R66 responde 'sair da pump.fun': pós-graduação custa igual ou mais e a regra perde 8/8 dias; antecipação achada e corrigida no próprio estudo → [[Diario/2026-09-22]]

**Pesquisa**

- `38d2ed09` research(meme): R65 — 87 operações reais: 72 % do prejuízo é CUSTO (rent de ATA 0,1135 = 33 %, nunca reembolsado); nenhuma das 13 variáveis de decisão separa vencedoras de perdedoras sob FDR; única pista buys_1m <= 25 (MFE +28 % vs +8 %), sem crédito estatístico; a regra atual tem 3 dias, não 6 → [[KB-0147-custo-e-o-prejuizo-e-buys-1m-e-a-unica-pista|KB-0147]]
- `4a030b86` research(meme): R66 — sair para pós-graduação NÃO é a saída barata: taxa 1,20 %/perna (só 5,8 % chegam à faixa de 0,30 %), ida e volta 2,40 % contra 2,23 % da curva; 86,8 % caem ≥ 30 % em 1 h; regra da mesa perde em 8 de 8 dias; nenhuma variável seleciona; PumpSwap é só saída no nosso código → [[KB-0148-a-graduacao-nao-e-a-saida-barata|KB-0148]]

## 2026-09-23

*(46 commits — o maior dia do intervalo; pesquisa (R67–R77) agrupada abaixo do trabalho de engenharia/ops/diário.)*

- `495bd117` diario(meme): 23/09 — RPC pago resolve a parada de 2 dias; T4.78 no ar; poda aplicada; RIGBY +0,0112; tudo no talo (300 moedas, spot/1 mode=on); R67 mata o candidato buys_1m fora da amostra
- `4d354bfb` feat(meme-worker): T4.80 + T4.79 (código) — critérios opcionais de portão: max_buys_1m (ausente por omissão; ceiling inclusivo; unknown recusa) e as features de absorção de venda (EXP-M22, pré-registro); trilha de recusas decodifica os dois; validação aceita int puro e recusa decimal/negativo; 1931 testes. A semente dos braços absorb (0058) fica para quando o Docker estiver disponível → [[EXP-M22-absorcao-de-venda|EXP-M22]]
- `042f856c` knowledge(meme): KB-0149 — consolidado dos 7 dias de mesa real: 28 achados com número e origem (custo, saída, entrada, infra, método) + o que continuamos sem saber → [[KB-0149-o-que-a-mesa-real-ensinou|KB-0149]]
- `d420deaf` diario(spot): 23/09 — primeiro sinal real da spot/1 (ZECUSDT) recusado por ata_account_via_lookup_table; T4.81 despachada para resolver as lookup tables
- `c0eb26d4` design(web): tela de confluência do mercado — gráfico + 4 faixas na mesma linha do tempo (Lab, notícias, nossa mesa, regime), painel 'neste instante' separando o que se sabia do que descobrimos depois; 5 overlays com regra de nunca-desenhar; API que falta e os dois briefs
- `a6bca761` diario(web): 23/09 — Everton autoriza market_events, exceção do dourado e janela ±15 min; briefs despachados
- `a41a3031` fix(spot): T4.81 — resolve as address lookup tables antes de verificar (a spot/1 recusava toda compra real: ata_account_via_lookup_table no 1.º sinal, ZECUSDT 11:45): estáticas + todos os graváveis + todos os somente-leitura, leitura em finalized, 13 recusas nomeadas novas, teto de tabelas/endereços/chaves; teste de mutação da ordem quebra 9; captura real da Jupiter no fixture; 707 testes no executor
- `f08a30a4` fix(meme): T4.83 — tesouraria resolve as lookup tables (estava inerte como a spot/1) e a derivação da ATA deixa de ser pulada quando o mint vem da tabela (vermelho primeiro: na captura real o índice 18 vinha None e qualquer mint era assinado sem checagem); 730 testes, mutação da ordem quebra 26; auditoria dos outros caminhos que assinam: o meme monta a própria mensagem e recusa v0 por construção
- `d9ccb961` exp(meme): EXP-M23 emenda 1 — amostragem estratificada (quase-falha 10 %/50 %, recusa múltipla 0,1 %/0,5 %) resolve a contradição entre população larga, taxa e alvo; contraste global ponderado pelo inverso, teste por critério só no estrato A → [[EXP-M23-desfecho-das-recusadas|EXP-M23]]
- `118c56dc` fix(meme): T4.84 — tesouraria: assinatura derivada e gravada antes do broadcast (envio ambíguo fica submitted, não failed sem assinatura — furava o teto diário de USDC), signer dentro do try (signer_failed:<Tipo>), reconcile lê o fill do meta da própria tx em vez de reler a carteira compartilhada (gravava confirmed com fill zero), expirado × erro on-chain com nomes distintos, resposta curta do RPC não condena linha; 749 testes, 7 mutantes mortos
- `8780211e` fix(spot): T4.86 — signer dentro do try nas duas pernas (signer_failed:<Tipo>, reserva liberada); _REFUSED aceita simulated (a correção seria no-op em produção — achado da Astra); falha do signer é transitória na saída, senão 6 indisponibilidades bloqueariam a venda de proteção; last_reconcile_result e medidor pending_unconfirmed/oldest_pending_s nas duas pistas; 767 testes, 11 mutantes mortos
- `78835dbb` feat(meme-worker): T4.85/EXP-M23 — sonda das recusadas (código; inerte sem a semente 0060): amostragem estratificada da emenda 1 (quase-falha 10/50 %, múltipla 0,1/0,5 %), semente congelada, probabilidade e estrato gravados na proposta, todos os motivos, guarda anti-antecipação com computed_at/tape_as_of; transação própria (não arrasta a mesa), pedigree da mesa subtrai as apostas da sonda (realimentação achada pela Astra), admissão por instante e não por lote; 544 testes → [[EXP-M23-desfecho-das-recusadas|EXP-M23]]
- `7a8e5b93` fix(meme): T4.8d — programa do pump.fun reancorado (slot 449734335, 23/09 11:45 BRT; IDL sha inalterado): discriminadores, ordem e writability das contas, creator_vault, fee_config/fee_program, TradeEvent e tier de taxas provados contra trades reais aterrados no programa novo + simulação mainnet (0 envios); mesa pode reabrir; asserção de 26 checks do T4.78 corrigida para 27
- `46e242a0` feat(db): migrações 0058 (braços de absorção EXP-M22), 0059 (market_events + guarda §17.7) e 0060 (sonda das recusadas EXP-M23) — verificadas com Postgres: 21 testes das migrações, cadeia upgrade/downgrade/upgrade limpa, cabeça única; guardas de downgrade agora escapam o texto interpolado (uma aspa em 'EXP-M23's' derrubava 42 testes e teria travado o deploy na migração); teste do 0059 aponta para a revisão certa depois da 0060 (regressão travada por teste sem Postgres em `1ce0694e`) → [[EXP-M22-absorcao-de-venda|EXP-M22]] · [[EXP-M23-desfecho-das-recusadas|EXP-M23]]
- `4c87a274` feat(api): T4.82 — API da tela de confluência: janela since/until nas velas e emitted_from/to nos sinais, GET /orgs/{org}/markets/{ex}/{sym}/desk (trilha da spot/1 por interseção de intervalo) e /events (market_events) atrás do mesmo portão do meme_live (portão vence o caminho: o registro do dinheiro real não fica atrás de porta mais fraca); script auditado market_event.py para o plantão; 23 testes de integração verdes com Postgres; truncagem explícita, único parcial por (source,url,symbol), market_id só quando inequívoco
- `fc72b40a` diario(meme): 23/09 tarde — programa do pump.fun mudou e o freio parou a mesa 1h50 (T4.8d reancorou com prova em trades reais); pacotes 11 e 12; Lab: poda concentrou o passivo, operator/5 é o pior braço ativo; T4.87 moinho de hipóteses → [[Diario/2026-09-23]]
- `d996e303` feat(research): T4.87 — moinho de hipóteses: run_hypothesis(spec) com pré-registro obrigatório (sem previsão e regra de refutação, recusa rodar), guarda anti-antecipação reutilizável, bootstrap por cluster e por bloco, permutação estratificada, BH/Holm, curva de limiares com diagnóstico patamar × pico e veredito CONFIRMA/NÃO CONFIRMA/REFUTA; reproduz exatamente os números publicados do R65 e do R67 como prova de confiabilidade; fila de 8 hipóteses no vault; 99 testes; 8 contraexemplos da Astra fechados (split furado, guarda contornável, NaN, cluster único, dependência temporal, fingerprint, grelha repetida, 'refutado' × 'não confirmado') → [[Fila de Hipoteses]]
- `a81dc660` fila(pesquisa): H-009 — giro rápido na oscilação (ideia do Everton, 23/09): vender o repique e recomprar a queda dentro dos 5 min, contra a regra atual, com 2,23 % de custo por ida e volta e 1,6 s de atraso → [[Fila de Hipoteses#H-009 — Giro rápido na oscilação (comprar a queda, vender o repique, repetir)|H-009]]
- `44f11a91` fila(pesquisa): H-010 — concentração do maior comprador, da perda real do AIRAA (a carteira que afundou estava na nossa fita 2 min antes; max_top10_share está vazio nas duas mesas) → [[Fila de Hipoteses#H-010 — Concentração do maior comprador (o dono que pode afundar)|H-010]]
- `85ec58c2` ficha(meme): 23/09 mesa real — 4 operações, 3 alvos, −0,0008; estratégia canalizada (entrada, saída, tamanho, custo) e as 11 métricas de decisão lado a lado; o MMKT bateu o alvo em 2 s a +63 %; nada separou o AIRAA, que morreu por concentração de dono (H-010) → [[Ficha-2026-09-23-mesa-real|Ficha 23/09]] · [[Fila de Hipoteses#H-010 — Concentração do maior comprador (o dono que pode afundar)|H-010]]
- `d73e67b5` fila(pesquisa): H-011 — onde deve ficar o alvo, o pedaço do R72 que sobreviveu (vender o primeiro repique e não voltar: +3,44 pp em 62 % das posições) → [[Fila de Hipoteses#H-011 — Onde deve ficar o alvo (vender o primeiro repique e não voltar)|H-011]]
- `48efe3cc` fila(pesquisa): H-013 — moeda em equilíbrio e não em subida (conjunção de fluxo fraco, vendas≈compras e curva rasa), da perda real do SHORT; lida de reasons, que é o que a porta gravou na decisão → [[Fila de Hipoteses#H-013 — Moeda em equilíbrio, não em subida (fluxo fraco + vendas ≈ compras + curva rasa)|H-013]]
- `2a8146f9` fix(meme): T4.88 — medido em 101 vendas reais: a escada de slippage NÃO causou a perda do AIRAA (os 77,6 % caíram num bloco com 14 vendas antes da 1.ª tentativa; os 6,4 s custaram 0,00008 SOL). O defeito real era a 'banda morta': uma venda falhada não era retentada quando o preço parava entre o alvo e o recuo (GAMON, 21/09: 17,4 s de espera, +0,0110 SOL recuperáveis com a mesma tolerância). Retentativa pendente ligada por padrão (MEME_EXIT_RETRY_MAX_ATTEMPTS=6, gatilho pela linha da ordem, teto com blocked_exits); escada por tentativa fica desligada (medição ambígua). 791 testes, mutação quebra 4. test_0041 reconhece as duas mesas (operator/5 e /6)
- `219c8e2b` fix(meme): T4.90 — reconcile relê a assinatura antes de declarar a venda expirada (uma venda que pousou no último bloco válido virava failed e a posição ficava aberta com o saldo divergente); reusa confirm.expired_or_landed (sem terceira variante), resposta curta do RPC nunca vira failed; com carteira vazia, antes de bloquear, pergunta à cadeia sobre até 3 vendas 'expiradas' e fecha pela que pousou; provado no Postgres de ponta a ponta, mutação quebra 6+2; teste do mapa spot passa a 38 ligados (0061)
- `7e7cefd9` docs(pesquisa): H-014 pré-registrada — rede coordenada de compradores (SIMFTR: 73 carteiras venderam no mesmo slot) antes de consultar financiadores (formato ajustado em `bfdee163`) → [[Fila de Hipoteses#H-014 — Rede coordenada de compradores (o golpe em um bloco só)|H-014]]
- `aa4ca4a4` docs(meme): ficha 23/09 — perda SIMFTR explicada (73 carteiras venderam no mesmo bloco; fluxo fabricado pela rede) → [[Ficha-2026-09-23-mesa-real|Ficha 23/09]]
- `ae501ca6` fix(meme): T4.90b — varredura de 30 s fecha posição aberta cuja venda já está confirmed (fill gravado lido por parser tipado, venue/lado/assinatura conferidos) e o tick nunca monta segunda venda sobre uma confirmada; spot: segunda resposta curta do RPC não vira failed; decodificador escolhido pela venue da ORDEM, não pela da posição (moeda que migra com venda pendente); cada posição isolada na varredura — fechamento recusado vira bloqueio com nome, não derruba os loops
- `db24dcc8` feat(meme): T4.89 — a pista de eventos grava a fita WS do instante da decisão (últimas 50 trocas, janelas 10/30/60 s, maior comprador líquido e maior saldo desde o nascimento provado pela curva, posição do criador) em reasons como evidência (used_by_gate=false) e em meme_decision_tapes (0062) por buffer assíncrono que nunca bloqueia nem falha a decisão; retenção 7 d trilha / 90 d proposta; MEME_DECISION_TAPE=off desliga
- `0d667b2b` docs(pesquisa): H-015 pré-registrada — SOL comprado no slot de criação, só em população nova (fita da T4.89b) → [[Fila de Hipoteses#H-015 — Compra no slot de criação (o "bundle" do lançamento)|H-015]]
- `219e3db2` feat(meme): T4.89b — fita da decisão v3 com o slot de cada troca e derived.creation_bundle (SOL e carteiras de terceiros no slot de criação, compra do criador à parte, early_slots dos 5 primeiros slots, create_signature, slot_source=first_trade_seen) para a H-015; limites de carteiras reaproveitam MAX_WALLETS/overflow; nenhuma migração, evidência só (used_by_gate=false) → [[Fila de Hipoteses#H-015 — Compra no slot de criação (o "bundle" do lançamento)|H-015]]
- `bbb7ffe0` docs(pesquisa): H-016 pré-registrada — entrar no recuo, não no pico (34 das 67 perdas reais nunca passaram do custo) (data em BRT corrigida em `684c2e63`) → [[Fila de Hipoteses#H-016 — Entrar no recuo, não no pico (esperar a primeira correção depois do sinal)|H-016]]
- `74219f71` docs(pesquisa): H-017 pré-registrada — recuo pequeno (3 %/60 s) como melhora de preço, só em coorte nova do braço recuo_v1 → [[Fila de Hipoteses#H-017 — Recuo pequeno como melhora de preço (coorte nova, braço de papel)|H-017]]

**Pesquisa (R67–R77)**

- `c403335c` research(meme): R67 — buys_1m NÃO confirma fora da amostra (473 mints independentes: D +0,036 [−0,058, +0,136], p 0,43; mediana com sinal errado; curva de limiares é pico e não planalto; ajuste em 19–20/09 dá sinal negativo no 25). Fica em sombra; nenhum parâmetro vai ao real. Sobrevive só o confound check
- `7405eef5` research(lab): R68 — prever a próxima vela não paga o pedágio: a 1 e 5 min o acerto necessário para empatar é >100 % ao custo da mesa (0,54 %); 25 células × 5 preditores, Holm: zero confirmações; 20/25 com IC superior < +10 bp (evidência contra); só 60/240 min são aritmeticamente possíveis e aí já não é a próxima vela → [[KB-0150-a-proxima-vela-e-menor-que-o-pedagio|KB-0150]]
- `a3cc4bdd` research(meme): R69 — percentil na coorte é o absoluto disfarçado (Spearman 0,81 mediano); 0 de 17 hipóteses passam BH e um controlo aleatório bate 6 das 13 reais; o portão já compra o percentil 0,96 sem saber; coorte fria não autoriza parar (IC cruza zero, cortaria 254 de 520 entradas); ponto cego desmentido: as não rastreadas graduam 3,3x MENOS e 98,7 % eram graduações instantâneas → [[KB-0151-a-coorte-nao-respira|KB-0151]] · [[EXP-M23-desfecho-das-recusadas|EXP-M23]]
- `9467d260` research: R70 — primeira rodada da fila: H-003 e H-004 NÃO CONFIRMAM (a do R69 reproduz ao 4.º decimal e morre na curva em pico), H-007 REFUTA, H-005 e H-008 voltam a abertas (pré-registo não cumprido: variável ausente no envelope; população verbatim = 0 linhas e censura de 51 %), H-001/H-002 sem amostra (14 e 0 apostas); H-006 expõe limite do moinho: selecionados×resto não é conservador e muda o veredito (tercil alto−baixo +0,23 % acima do MRE); toda a fila do lado à vista mede 'qual sub-braço perde menos'; teste da fila deixa de ancorar em open_hypotheses → [[Fila de Hipoteses]]
- `c482cbca` research(meme): R72/H-009 — a oscilação existe (mediana de 3,5 giros de 3 % por posição em 5 min) mas o giro NÃO paga: 0 de 12 células confirmam, todos os IC atravessam zero, menor p 0,12; equilíbrio por giro = 2,22 % contra custo de 2,23 %, sobrando 0,00055 SOL por giro; 11-15 % das recompras caem em dreno e a política sem repique perde 31,8 %; medida numa versão otimista de si mesma e ainda assim não vence → [[KB-0152-a-oscilacao-existe-o-giro-nao-paga|KB-0152]] · [[Fila de Hipoteses#H-009 — Giro rápido na oscilação (comprar a queda, vender o repique, repetir)|H-009]]
- `02ce1c32` research(meme): R73/H-010 — concentração do maior comprador NÃO CONFIRMA por limite de dado: cobertura desde o nascimento 53,8 % (abaixo dos 60 % da própria regra) e 1 de 91 com a guarda de chegada; SENTHOS (+19,6 %) tinha o criador com 68 % da curva — um teto de concentração bloquearia o vencedor; não configurar max_top10_share. Achado maior: meme_trades é cópia por polling com ~44 s de atraso (p90 129 s) e a fita WS que decide não é gravada — features de decisão derivadas de meme_trades não medem o que a mesa sabia → [[KB-0153-o-maior-comprador-nao-estava-no-arquivo|KB-0153]] · [[Fila de Hipoteses#H-010 — Concentração do maior comprador (o dono que pode afundar)|H-010]]
- `43c8f36c` research(meme): R74/H-011+H-012 — nenhum alvo nem tempo máximo bate a regra atual (REFUTA nas duas populações): alvo 1,08 fica na borda e seu ganho vive nas posições com buraco na fita (−3,06 pp nas bem cobertas); o fragmento +3,44 pp do R72 era seleção pelo resultado (some quando medido em todas); subir para 1,50 cria perdas graves; encurtar para 30 s corta 14 de 23 vitórias; o 1,15 já captura parte da corrida (mediana realizada +17,6 %, máx. +80 %) → [[KB-0154-subir-o-alvo-nao-paga|KB-0154]] · [[Fila de Hipoteses#H-011 — Onde deve ficar o alvo (vender o primeiro repique e não voltar)|H-011]] · [[Fila de Hipoteses#H-012 — Tempo máximo curto ("o que não sobe logo não sobe mais")|H-012]]
- `017d4b3b` feat(spot): R71 — migração 0061 amplia o mapa da spot/1 em 6 mercados provados por paridade com a Binance e pela ponte na fonte (NEAR via OmniBridge, BTC via Wormhole): NEAR, BTC e ORCA ligados; XRP (emissor pode congelar a conta) e BIRB (90,9 % nos maiores detentores) retidos por decisão, SLX desligado por regra (tier C). DASH, o maior buraco (121 sinais em 14 dias), não tem token válido na Solana — os dois 'DASH' da Jupiter são memecoins a 0,00026 US$. Ampliação: +3 sinais v14/semana (+7 %); 7 testes da migração com Postgres
- `a6dfede0` research(meme): R75/H-013 — moeda em equilíbrio NÃO CONFIRMA por limite de dado: em 587 decisões só 1 tem a conjunção (o próprio SHORT); 502 vêm de braços com teto 0,6 que a excluem por construção; no operator/5 (teto 1,0) a taxa é 1 em 85 e chegar a 20 casos pede ~1 700 decisões. Isoladas, as três condições apontam em direções opostas; contrafactual remove só o caso de origem (circular). Lido de reasons: cobertura 100 %, sem look-ahead → [[KB-0155-o-equilibrio-quase-nao-passa-na-porta|KB-0155]] · [[Fila de Hipoteses#H-013 — Moeda em equilíbrio, não em subida (fluxo fraco + vendas ≈ compras + curva rasa)|H-013]]
- `25539317` docs(pesquisa): R76 — H-014 NÃO CONFIRMA: o despejo em bloco não é rede de financiamento (SIMFTR rede 0 %; 98 % dos financiadores resolvidos via Helius; teto de rede bloqueia 1 das 6 piores e mata vencedoras); pista exploratória: SOL comprado no slot de criação → [[KB-0156-o-despejo-em-bloco-nao-e-uma-rede-de-financiamento|KB-0156]] · [[Fila de Hipoteses#H-014 — Rede coordenada de compradores (o golpe em um bloco só)|H-014]]
- `3cb6b80c` docs(pesquisa): R77 — H-016 REFUTA (a)(b): esperar o recuo não paga; melhor célula X=3 %/60 s na borda, +2,0 pp reais IC [−3,6, +7,5], +2,3 pp papel IC [+0,7, +3,9] Holm 0,052 — ganho de preço, não de evitar moedas; as moedas que não recuam são as vencedoras; controle modelado +0,41 % vs real −5,42 % (aluguel 2,8 pp + execução 3,2 pp) → [[KB-0157-esperar-o-recuo-nao-paga|KB-0157]] · [[Fila de Hipoteses#H-016 — Entrar no recuo, não no pico (esperar a primeira correção depois do sinal)|H-016]]

## 2026-09-24

- `b92f4f99` feat(meme): T4.91 — entrada no recuo na pista de eventos (desligada sem entry_pullback_pct: proposta idêntica) e braço de papel recuo_v1/1 (0063, research_only, cópia da porta viva do operator/5, 3 %/60 s) para a H-017/EXP-M24; rechecagem no gatilho mata venda do criador e toda recusa fora da lista de dispensas; não-entradas nomeadas na trilha (no_pullback, killed, censored); inserção do braço fora do laço de avaliação (BoundedTasks, 8 em voo); carregador de rule sets pula linha inválida sem derrubar a mesa; apostas do braço não fixam mints no rastreador (não alteram o pedigree do operator/5) → [[EXP-M24-entrada-no-recuo|EXP-M24]] · [[Fila de Hipoteses#H-017 — Recuo pequeno como melhora de preço (coorte nova, braço de papel)|H-017]]
- `954f0a50` perf(meme): pedigree do minuto só é lido quando há rule set no relógio 1m (nenhum ativo: a leitura de ~14 s estourava o limite de 8 s em ~40 % dos minutos e atrasava ~8 s o cache da pista de eventos); índice parcial meme_tokens(symbol, created_at) na 0064 (CONCURRENTLY; symbol_dup_24h era 74 % do custo); falha de leitura loga sqlstate/detail
- `76f9ff0a` docs(pesquisa): H-018 pré-registrada — recompra do mesmo mint logo depois de um ganho (Megawatt −78,7 % 5 s após o alvo) → [[Fila de Hipoteses#H-018 — Recompra do mesmo mint logo depois de um ganho|H-018]]

*(o diário de 24/09 só foi redigido em 25/09, junto com o de 25/09, por `d2e4e9fe` — ver seção seguinte.)* → [[Diario/2026-09-24]]

## 2026-09-25

- `d2e4e9fe` docs(diario): 24/09 (pior dia, R76/R77, recuo no ar, pausa não aplicada) e 25/09 (primeiro dia verde, recompras custaram 0,041, primeira compra real da spot/1 em TAO, placar dos braços) → [[Diario/2026-09-24]] · [[Diario/2026-09-25]]
- `ce33e1e2` docs(memoria): estratégia passa pelo Obsidian antes de ser usada ou mudada (regra do Everton, 25/09)
- `ca608dd3` feat(web): placar dos braços de pesquisa meme em Mesa → Testes — um cartão por rule set vindo do banco (recuo_v1/1 ao lado do operator/5), com parâmetros do recuo, apostas, acertos, SOL e R médio sobre fechamentos avaliáveis; /lab aponta para ele (o Lab cripto e o meme são domínios diferentes)
- `36d2c3a0` feat(ops): T4.92 — ficha automática diária da mesa real no Obsidian (SELECT-only; posição por posição com pico, 'subiu depois da compra?', custos, fita da decisão e classe de perda automática: comprou_no_topo / golpe_do_criador / recompra / custo / saida_normal; spot/1; placar dos braços; --week por classe); fichas de 24 e 25/09 geradas da VPS → [[Ficha-2026-09-24|Ficha 24/09]] · [[Ficha-2026-09-25|Ficha 25/09]]
- `8840f397` feat(ops): T4.93 — portão 'Obsidian primeiro': meme_rule_set (--set-param/--deprecate), activate_strategy_version (ativação e --paper-line) e spot_desk_markets (--enable/--disable/--set-mint) recusam --apply sem --note obsidian/... que cite exatamente o alvo (fronteira de identificador, um grupo por alvo); proveniência (caminho, sha256, blob git dos mesmos bytes) gravada onde a ferramenta já registra; ops monta obsidian/ só leitura; regra .claude/rules/obsidian-first.md e linha no CLAUDE.md
- `297658be` fix(meme): T4.94 — proposta cujo mint teve atividade depois que ela nasceu (posição aberta, saída ou fechamento pelo executor, ou ordem de outra proposta) é rejeitada como mint_busy_superseded, de forma durável; gêmeas da proposta escolhida rejeitadas na mesma passada; frescor da aprovação automática 60 → 10 s (MEME_LIVE_AUTO_APPROVE_MAX_AGE_S), relógio monotônico depois do refresh do kill switch; too_old contado por pista
- `d2a213f0` docs(ficha): 24/09 regerada pela imagem implantada (297658be) — prova do caminho ops → [[Ficha-2026-09-24|Ficha 24/09]]
- `28272a06` docs(pesquisa): H-019 pré-registrada — fluxo desacelerando na hora da compra (janela de 10 s da fita da decisão contra o maior vazamento, comprou_no_topo) → [[Fila de Hipoteses#H-019 — Fluxo desacelerando na hora da compra (o topo local visto pela fita de 10 s)|H-019]] · [[comprou_no_topo]]
- `53a1295e` feat(ops): ficha diária/semanal com campos Dataview no frontmatter (dia, operações, ganhos, pnl_sol, perdas por classe n/sol, maior_vazamento; semana_inicio/fim) para o Painel da mesa e as páginas de perda → [[Painel da mesa]] · [[Index|Perdas]]
- `7f217192` docs(obsidian): Painel da mesa (Tracker: resultado diário, acumulado, acerto, SOL por classe de perda; Dataview dos últimos 14 dias) e modelos do Templater em _templates (nova hipótese no formato do moinho, nova ideia, nova nota de pesquisa) → [[Painel da mesa]]
- `c0abd622` docs(obsidian): base preparada para criar estratégias — Dicionário de Variáveis (53, 30 nunca testadas), Mapa de Estratégias (vivas/pistas/cemitério via Dataview + tabela estática), páginas por classe de perda, Ideias do Everton; frontmatter padronizado (tipo, hipótese, variável, população, efeito, IC, veredito, próximo passo, classe de perda, mercado) em 206 notas KB/EXP; R79 (KB-0159, vereditos H-017/H-019, avaliação datada do EXP-M24) → [[Dicionario de Variaveis]] · [[Mapa de Estrategias]] · [[Index|Perdas]] · [[Ideias do Everton]] · [[KB-0159-a-desaceleracao-nao-avisa-o-topo|KB-0159]] · [[Fila de Hipoteses#H-017 — Recuo pequeno como melhora de preço (coorte nova, braço de papel)|H-017]] · [[Fila de Hipoteses#H-019 — Fluxo desacelerando na hora da compra (o topo local visto pela fita de 10 s)|H-019]] · [[EXP-M24-entrada-no-recuo|EXP-M24]]
- `d4517dc0` docs(obsidian): páginas de perda somam as fichas pelo Dataview (perdas_<classe>_n/_sol); Painel da mesa ligado ao índice de perdas → [[Painel da mesa]] · [[Index|Perdas]]
- `40cfa93b` feat(meme): T4.95 — braço de controle recuo_ctrl_v1/1 (0065, research_only, EXP-M25): os parâmetros vivos do recuo_v1/1 sem o recuo, entrada imediata em t0, para que toda armação do recuo tenha par na mesma porta (H-017 deixa de depender da mesa real aceitar: 39 → todas as armações); apostas do controle fora do pedigree e dos pins do operator/5; executor nunca o escolhe → [[EXP-M25-controle-do-recuo|EXP-M25]] · [[Fila de Hipoteses#H-017 — Recuo pequeno como melhora de preço (coorte nova, braço de papel)|H-017]]

**Pesquisa (R78–R79)**

- `9d9ce5f5` docs(pesquisa): R78 — H-018 limite de dado (10 recompras após ganho, 9 perderam, −0,118 SOL; pausa após qualquer saída teria dado +0,117 matando só KODA) e H-015 REFUTA (c) (teto do tercil alto mata 36,5 % das vencedoras); achado: a fila mint_busy aprova proposta de até 60 s sem reler a fita (007, BAGI, ANT); slot de criação inferido erra em 36/363 (formato do campo de status corrigido em `c48325ea`) → [[KB-0158-recompra-sem-amostra-e-bundle-sem-filtro|KB-0158]] · [[Fila de Hipoteses#H-018 — Recompra do mesmo mint logo depois de um ganho|H-018]] · [[Fila de Hipoteses#H-015 — Compra no slot de criação (o "bundle" do lançamento)|H-015]]
- `740728dd` docs(pesquisa): R79 — H-019 REFUTA (c): desaceleração do fluxo não avisa o topo (sinal contrário; piso mataria 6 de 21 vencedoras reais, Δ −0,007 SOL); H-017 limite de dado (39 pares com controle; +0,032 [−0,006, +0,082]); papel preenche na foto seguinte — 22 de 34 pares compraram na mesma foto → [[Fila de Hipoteses#H-019 — Fluxo desacelerando na hora da compra (o topo local visto pela fita de 10 s)|H-019]] · [[Fila de Hipoteses#H-017 — Recuo pequeno como melhora de preço (coorte nova, braço de papel)|H-017]] (KB-0159 publicada em `c0abd622`, acima)
